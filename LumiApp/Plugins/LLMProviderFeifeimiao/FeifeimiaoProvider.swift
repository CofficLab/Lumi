...
final class FeifeimiaoProvider: NSObject, SuperLLMProvider, @unchecked Sendable {
    ...
    var baseURL: String {
        "https://api.feifeimiao.top/v1/chat/completions"
    }
    ...
    func parseResponse(data: Data) throws -> (content: String, toolCalls: [ToolCall]?) {
        let result = try JSONDecoder().decode(FeifeimiaoResponse.self, from: data)

        guard let choiceMessage = result.choices.first?.message else {
            throw NSError(domain: "FeifeimiaoProvider", code: -1, userInfo: [NSLocalizedDescriptionKey: "No choices in response"])
        }
...
private struct FeifeimiaoResponse: Decodable {
    let choices: [Choice]
