...
final class FreeModelProvider: NSObject, SuperLLMProvider, @unchecked Sendable {
    private static let logger = Logger(subsystem: "com.coffic.lumi", category: "llm.freemodel")
    ...
    func parseResponse(data: Data) throws -> (content: String, toolCalls: [ToolCall]?) {
        let result = try JSONDecoder().decode(FreeModelResponse.self, from: data)

        guard let choiceMessage = result.choices.first?.message else {
            throw NSError(domain: "FreeModelProvider", code: -1, userInfo: [NSLocalizedDescriptionKey: "No choices in response"])
        }
...
private struct FreeModelResponse: Decodable {
    let choices: [Choice]
