...
/// 记忆注入中间件。
///
/// 保留在 App 层，因为它依赖 `WindowProjectVM` 获取项目路径和语言偏好。
/// 使用 PluginMemory 包中的 Service 进行记忆读写。
@MainActor
final class MemoryContextSuperSendMiddleware: SuperSendMiddleware, SuperLog {
    ...
    func handle(
        ...
        let config = PluginMemory.MemoryPlugin.config
        ...
        if injectGlobal {
            globalIndex = await PluginMemory.MemoryStorageService.shared.readIndex(scope: .global)
            let userMessage = ctx.message.content
            globalRelevant = await PluginMemory.MemoryRetrievalService.shared.findRelevant(
                query: userMessage,
                scope: .global,
        ...
        if !projectPath.isEmpty && injectProject {
            ...
            projectIndex = await PluginMemory.MemoryStorageService.shared.readIndex(scope: projectScope)
            let userMessage = ctx.message.content
            projectRelevant = await PluginMemory.MemoryRetrievalService.shared.findRelevant(
                query: userMessage,
                scope: projectScope,
                maxResults: maxRelevant
            )
