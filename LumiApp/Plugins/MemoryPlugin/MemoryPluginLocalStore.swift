...
///
/// 负责管理插件的配置持久化。
final class MemoryPluginLocalStore: @unchecked Sendable {
    static let shared = MemoryPluginLocalStore()
    ...
    func integer(forKey key: Key, defaultValue: Int = 0) -> Int {
        (object(forKey: key) as? Int) ?? defaultValue
    }

    func double(forKey key: Key, defaultValue: Double = 0) -> Double {
    ...
    var shouldInjectGlobalIndex: Bool {
        bool(forKey: .injectGlobalIndex, defaultValue: true)
    }

    var shouldInjectProjectIndex: Bool {
        bool(forKey: .injectProjectIndex, defaultValue: true)
    }

    var autoSaveOnContextSwitch: Bool {
