...
struct EditorResolvedPackagePin: Equatable, Sendable {
    let identity: String
    let location: String
    let version: String?
    let branch: String?
...
enum EditorPackageResolved {
    static func parse(url: URL) throws -> [EditorResolvedPackagePin] {
        let data = try Data(contentsOf: url)
        return try parse(data: data)
    }

    static func parse(data: Data) throws -> [EditorResolvedPackagePin] {
    ...
    static func identityFromLocation(_ location: String) -> String {
