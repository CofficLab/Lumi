...
enum EditorFileTreeGitStatus: String, CaseIterable, Sendable {
    case modified = "M"
    case added = "A"
    case deleted = "D"
    case renamed = "R"
    ...
    /// 取两个状态中优先级更高的那个
    static func highest(_ a: EditorFileTreeGitStatus, _ b: EditorFileTreeGitStatus) -> EditorFileTreeGitStatus {
        a.priority >= b.priority ? a : b
    }
}

/// 单个文件的 Git 状态条目
struct EditorFileTreeGitStatusEntry: Sendable {
    /// 相对于仓库根目录的 POSIX 路径
    let relativePath: String
    /// 文件状态
    let status: EditorFileTreeGitStatus
    /// 是否已暂存
    let isStaged: Bool

    init(relativePath: String, status: EditorFileTreeGitStatus, isStaged: Bool = false) {
...
/// Git 状态快照，供文件树视图只读查询
struct EditorFileTreeGitStatusSnapshot: Sendable {
    /// 文件路径 → 状态条目（相对路径为 key）
    let entriesByRelativePath: [String: EditorFileTreeGitStatusEntry]

    /// 目录路径 → 聚合的最高优先级状态（用于文件夹行显示）
    let directoryAggregateByRelativePath: [String: EditorFileTreeGitStatus]
    ...
    static let empty = EditorFileTreeGitStatusSnapshot(
    ...
    /// 查询指定相对路径的文件状态
    func statusForPath(_ relativePath: String) -> EditorFileTreeGitStatus? {
        entriesByRelativePath[relativePath]?.status
    }

    /// 查询指定目录的聚合状态
    func aggregateStatusForDirectory(_ relativePath: String) -> EditorFileTreeGitStatus? {
...
final class EditorFileTreeGitStatusProvider: @unchecked Sendable, SuperLog {
    ...
    func captureSnapshot(projectRootPath: String) -> EditorFileTreeGitStatusSnapshot? {
        // 1. 检测是否为 Git 仓库
