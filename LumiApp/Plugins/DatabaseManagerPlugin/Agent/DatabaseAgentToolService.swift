...
struct DatabaseConnectionSummary: Encodable {
    let id: String
    let name: String
    let type: String
    let host: String?
...
struct DatabaseQueryPayload: Encodable {
    let columns: [String]
    let rows: [[String]]
...
enum DatabaseAgentToolError: LocalizedError {
...
actor DatabaseAgentToolService {
    static let shared = DatabaseAgentToolService()
    static let maxRows = 1_000
    static let defaultRows = 100
    ...
    func describeSchema(connectionId: UUID, limit: Int) async throws -> String {
    ...
    func queryReadonly(connectionId: UUID, sql: String, limit: Int) async throws -> String {
        let limit = try Self.normalizedLimit(limit)
        let config = try await config(for: connectionId)
    ...
    func sampleTable(connectionId: UUID, table: String, limit: Int) async throws -> String {
        let limit = try Self.normalizedLimit(limit)
        let config = try await config(for: connectionId)
    ...
    static func connectionId(from value: Any?) throws -> UUID {
    ...
    static func normalizedLimit(_ value: Any?) throws -> Int {
    ...
    static func normalizedLimit(_ limit: Int) throws -> Int {
    ...
    static func readonlySQL(_ sql: String, type: DatabaseType, limit: Int) throws -> String {
    ...
    static func rowsAsDictionaries(_ result: QueryResult, limit: Int) -> DatabaseQueryPayload {
    ...
    static func quoteIdentifierPath(_ path: String, type: DatabaseType) -> String {
        path.split(separator: ".")
            .map { quoteIdentifier(String($0), type: type) }
            .joined(separator: ".")
    }

    static func quoteIdentifier(_ identifier: String, type: DatabaseType) -> String {
        switch type {
        case .mysql:
    ...
    static func valueString(_ value: DatabaseValue) -> String {
        let raw: String
        switch value {
    ...
    static func encodeJSONObject(_ object: Any) throws -> String {
