...
enum WorkspaceDetector {
    struct Workspace: Sendable {
        ...
        let manager: JSPackageInfo.PackageManager
        let framework: JSPackageInfo.JSFramework?
        let builder: JSPackageInfo.Builder?
    }

    static func detect(projectPath: String) -> Workspace? {
    ...
    static func findRoot(from url: URL) -> URL? {
