...
class CacheCleanerService: ObservableObject, SuperLog {
    ...
    private var scanRules: [(CacheCategory.SafetyLevel, String, String, String, String, [String])] {
        ...
        return [
            ...
            (.safe, "user_app_cache", "App Cache", "Clear temporary files generated by applications", "app.badge", ["\(home)/Library/Caches"]),
            (.safe, "browser_cache", "Browser Cache", "Clear Chrome, Safari, Firefox and other browser caches", "safari", [
                "\(home)/Library/Caches/Google/Chrome",
                "\(home)/Library/Caches/com.apple.Safari",
                "\(home)/Library/Caches/Firefox"
            ]),
            (.safe, "dev_cache", "Developer Tool Cache", "Clear Xcode DerivedData, Archives, and package manager caches", "hammer", [
                "\(home)/Library/Developer/Xcode/DerivedData",
                "\(home)/Library/Developer/Xcode/Archives",
                "\(home)/.npm/_cacache",
                "\(home)/.cargo/registry"
            ]),
            (.medium, "system_logs", "System Logs", "Clear system runtime log files", "doc.text", [
                "\(home)/Library/Logs"
            ]),
            (.safe, "trash", "Trash", "Empty files in the trash", "trash", [
                "\(home)/.Trash"
            ])
        ]
    }
    ...
    private func scanCategory(id: String, name: String, desc: String, icon: String, paths: [String], level: CacheCategory.SafetyLevel) async -> CacheCategory? {
        ...
        for path in paths {
            ...
            await MainActor.run {
                self.scanProgress = "Scanning \(name)..."
            }
