...
class CacheCleanerService: ObservableObject, SuperLog {
    ...
    private var scanRules: [(CacheCategory.SafetyLevel, String, String, String, String, [String])] {
        ...
        return [
            ...
            (.safe, "user_app_cache", String(localized: "App Cache", table: "DiskManager"), String(localized: "Clear temporary files generated by applications", table: "DiskManager"), "app.badge", ["\(home)/Library/Caches"]),
            (.safe, "browser_cache", String(localized: "Browser Cache", table: "DiskManager"), String(localized: "Clear Chrome, Safari, Firefox and other browser caches", table: "DiskManager"), "safari", [
                "\(home)/Library/Caches/Google/Chrome",
                "\(home)/Library/Caches/com.apple.Safari",
                "\(home)/Library/Caches/Firefox"
            ]),
            (.safe, "dev_cache", String(localized: "Developer Tool Cache", table: "DiskManager"), String(localized: "Clear Xcode DerivedData, Archives, and package manager caches", table: "DiskManager"), "hammer", [
                "\(home)/Library/Developer/Xcode/DerivedData",
                "\(home)/Library/Developer/Xcode/Archives",
                "\(home)/.npm/_cacache",
                "\(home)/.cargo/registry"
            ]),
            (.medium, "system_logs", String(localized: "System Logs", table: "DiskManager"), String(localized: "Clear system runtime log files", table: "DiskManager"), "doc.text", [
                "\(home)/Library/Logs"
            ]),
            (.safe, "trash", String(localized: "Trash", table: "DiskManager"), String(localized: "Empty files in the trash", table: "DiskManager"), "trash", [
                "\(home)/.Trash"
            ])
        ]
    }
    ...
    func scanCaches() async {
        isScanning = true
        scanProgress = String(localized: "Initializing...", table: "DiskManager")
    ...
    private func scanCategory(id: String, name: String, desc: String, icon: String, paths: [String], level: CacheCategory.SafetyLevel) async -> CacheCategory? {
        ...
        for path in paths {
            ...
            await MainActor.run {
                self.scanProgress = String(localized: "Scanning \(name)...", table: "DiskManager")
            }
        ...
        for content in contents {
            if let info = await getPathInfo(content.path), info.size > 0 {
                results.append(CachePath(
                    ...
                    description: String(localized: "App Cache", table: "DiskManager"),
                    size: info.size,
                    fileCount: info.count,
                    canDelete: true
                ))
