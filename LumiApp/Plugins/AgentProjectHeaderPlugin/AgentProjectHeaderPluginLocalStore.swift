...
final class AgentProjectHeaderPluginLocalStore: @unchecked Sendable {
    private let fileManager = FileManager.default
    private let queue = DispatchQueue(label: "AgentProjectHeaderPluginLocalStore.queue", qos: .userInitiated)
    private let settingsDirectory: URL
    private let settingsFileURL: URL

    init() {
        let root = AppConfig.getDBFolderURL()
            .appendingPathComponent("AgentProjectHeaderPlugin", isDirectory: true)
