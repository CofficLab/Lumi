...
struct ConversationHandoffSummaryService {
    private let maxSourceCharacters = 24_000

    func summarize(messages: [ChatMessage], config: LLMConfig, llmService: LLMService) async throws -> String {
        ...
        let response = try await llmService.sendMessage(messages: [request], config: config, tools: [])
    ...
    func handoffMessage(from summary: String) -> String {
        """
...
enum ConversationHandoffSummaryError: LocalizedError {
