...
/// - 如果没有最近项目或项目为空，则不注入
@MainActor
final class RecentProjectsSendMiddleware: SuperSendMiddleware, SuperLog {
    nonisolated static let emoji = "📋"
    nonisolated static let verbose: Bool = true
    let id: String = "recent-projects-context"
    /// 优先级设为 -5，在语言中间件之后执行
    let order: Int = -5
        ...
        if !projects.isEmpty {
            ...
            if Self.verbose {
                if RecentProjectsPlugin.verbose {
                    RecentProjectsPlugin.logger.info("📋 最近项目中间件：注入 \(projects.count) 个项目")
                }
            }
        }
