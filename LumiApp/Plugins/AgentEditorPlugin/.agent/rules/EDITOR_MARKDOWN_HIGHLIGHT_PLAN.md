...
1.  **内核层**：在 `EditorExtensionContributors.swift` 和 `EditorExtensionRegistry.swift` 中添加高亮提供者注册能力。
2.  **桥接层**：在 `SourceEditorView.swift` 中注入插件高亮提供者到 `highlightProviders` 数组。
3.  **插件层**：创建 `MarkdownEditorPlugin`，实现基于正则的 `HighlightProviding`。
4.  **性能验证**：确保在大文件和快速滚动场景下，高亮逻辑符合 Phase 8 的指标。
