...
| Phase | 内容 | 状态 |
|-------|------|------|
| Phase 4 | EditorGroup + WorkbenchState + split editor | 🟢 基本完成 |
| Phase 5 | Central command registry + keybinding | 🟢 基本完成 |
| Phase 6 | Language Pipelines (request lifecycle, cancellation) | 🟡 持续推进中 |
| Phase 8 | Performance / Large File | 🟡 已启动 |
| Phase 9 | Polish (bracket pair, auto-closing, save participants) | 🟡 已启动 |
...
### Phase 6: Language Pipelines — 🟡 持续推进中

**已接入统一 request lifecycle / stale rejection 的真实 consumer：**
- `LSPInlayHintEditorPlugin/InlayHintProvider.swift`
- `LSPDocumentHighlightEditorPlugin/DocumentHighlightProvider.swift`
- `LSPCodeActionEditorPlugin/CodeActionProvider.swift`
- `LSPSignatureHelpEditorPlugin/SignatureHelpProvider.swift`
- `LSPWorkspaceSymbolEditorPlugin/WorkspaceSymbolProvider.swift`
- `LSPSelectionRangeEditorPlugin/SelectionRangeProvider.swift`
- `LSPDocumentLinkEditorPlugin/DocumentLinkProvider.swift`
- `LSPDocumentColorEditorPlugin/DocumentColorProvider.swift`
- `LSPCallHierarchyEditorPlugin/CallHierarchyProvider.swift`
- `LSPFoldingRangeEditorPlugin/FoldingRangeProvider.swift`

**已收口的高频实时链路：**
- `LSPRealtimeSignalsEditorPlugin/HoverCoordinator.swift`
- `LSPRealtimeSignalsEditorPlugin/LSPCompletionDelegate.swift`
- `LSPContextCommandsEditorPlugin/JumpToDefinitionDelegate.swift`
- `EditorState.showReferencesFromCurrentCursor()`
- `LSPServiceEditorPlugin/LSPCoordinator.swift` 的 semantic token refresh

**已收口的 transport 层语义：**
- `LSPCoordinator` 文件切换时会 `cancelAll()` 防抖/节流请求
- direct request / debounced request / throttled request 都会做 file-session stale rejection

### Phase 9: Polish — 🟡 已启动

**save participants 第一版已落地：**
- `Kernel/EditorSaveParticipantController.swift`
- `Kernel/EditorSavePipelineController.swift`
- 保存前自动执行：
  - trim trailing whitespace
  - insert final newline
- `EditorState.saveNow()` 已改成统一走 save pipeline
- pipeline 已接入 `format on save` 异步入口，默认关闭
- pipeline 已接入保守版 `code action on save`：
  - `organize imports on save`
  - `fix all on save`
  - 当前只应用 `WorkspaceEdit`，不执行任意 command
- `EditorSavePipelineResult` 现在会显式产出 deferred save actions
  - `EditorState` 不再自己硬编码组织 save code action 请求
  - 后续继续增加保存动作时，只需要扩展 pipeline 输出和执行器
- 保存行为现在已有持久化配置：
  - `format on save`
  - `organize imports on save`
  - `fix all on save`
  - `trim trailing whitespace`
  - `insert final newline`
- `EditorToolbarView` 已提供保存行为菜单入口
- 保存前会先将变换后的文本回写到 `documentController` / `NSTextStorage` / LSP snapshot，再执行落盘
- 外部文件修改冲突现在不再静默忽略：
  - 本地有脏改动时，检测到外部修改会进入显式 conflict state
  - `EditorToolbarView` 提供 `Reload from Disk / Keep Editor Version` 处理入口
- 对应测试：
  - `Tests/EditorSaveParticipantControllerTests.swift`
  - `Tests/EditorSavePipelineControllerTests.swift`
...
- `LSPCallHierarchyEditorPlugin/CallHierarchyProvider.swift`
- `LSPFoldingRangeEditorPlugin/FoldingRangeProvider.swift`
- `LSPServiceEditorPlugin/LSPCoordinator.swift` 中的 `SemanticTokenHighlightProvider`
- `LSPRealtimeSignalsEditorPlugin/HoverCoordinator.swift`
- `LSPRealtimeSignalsEditorPlugin/LSPCompletionDelegate.swift`
- `LSPContextCommandsEditorPlugin/JumpToDefinitionDelegate.swift`
- `EditorState.showReferencesFromCurrentCursor()` 当前文件引用查询

**当前效果：**
1. 快速输入、快速滚动、连续触发导航时，旧结果不会覆盖新状态
2. 同一条语言能力在文件切换 / session 切换 / group host 切换后更不容易回写旧结果
3. provider 级查询和一部分高频 realtime/editor consumer 已经开始共享同一套 request generation 语义

**下一步（Phase 6.5）：**
1. 继续检查是否还有直接写 UI 状态、但未走 shared stale rejection 的高频链
2. 补 request lifecycle 的细粒度测试，而不只是保留基础设施测试
3. 继续把 transport concerns 和 presentation concerns 分离，减少 provider 里直接拼 UI 状态的代码
...
| 运行时模式接线 | `EditorState.loadFile/resetState/loadBinaryFile` 已计算并维护 `largeFileMode` |
| 功能自动降级 | `SourceEditorView` 已对 semantic tokens / inlay hints / folding / minimap 做 runtime gating |
| Viewport 运行时接线 | `ScrollCoordinator -> EditorState.viewportRenderController` 已维护可见行区间和渲染缓冲区 |
| 语法高亮上限 | `maxSyntaxHighlightLines` 已开始按 viewport 行区间 gating tree-sitter / semantic provider |
| Inlay viewport 调度 | viewport 变化已驱动 inlay hint 请求调度与残留 hint 清理 |
| Semantic viewport 暂停/恢复 | semantic token provider 已随 viewport gating 清空旧结果并在恢复时主动重刷 |
| 长行保护 | `LongLineDetector.findLongestLine()` 检测 >10,000 字符的超长行 |
| 长行运行时状态 | `EditorState` 已暴露 `longestDetectedLine` |
| 长行运行时降级 | 超长行 + large/mega 模式下会抑制语法高亮与 inlay hint 运行时链 |
| 截断安全 | 仍保留 `>2MB` 截断预览，避免截断内容可编辑/可保存 |

**待继续收口：**
1. `ViewportRenderController` 还未进一步驱动更细粒度的按需渲染或 LSP viewport 刷新
2. `maxSyntaxHighlightLines` 已扩到语义高亮和 inlay hint，但还没覆盖更多高成本实时能力
3. 长行保护已进入运行时 gating，但还没做更细的逐功能降级策略
