...
enum EmmetConfig {
    ...
    /// 默认使用的变量映射
    static let defaultVariables: [String: String] = [
        "lang": "zh-CN",
        "charset": "UTF-8",
        "locale": "zh-CN",
    ]
    ...
    /// 不同语法的缩进配置
    static let indentStyle: String = "  " // 两空格缩进

    /// 是否启用 inline 模式（单行输出）
    static let inlineMode: Bool = false
...
"""
    ...
    static let jsxMode: Bool = false

    // MARK: - 方法

    /// 获取展开语法模式
    static func syntaxMode(for languageId: String) -> EmmetSyntax {
    ...
    static func isEnabled(at context: EmmetContext) -> Bool {
        switch context {
        case .html:
            return true
        case .css:
