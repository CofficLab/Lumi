...
/// - 属性值以 `/` 或 `./` 或 `../` 开头
enum HTMLPathCompletion {
    /// 需要路径补全的属性列表
    static let pathAttributes: Set<String> = ["src", "href", "action", "data", "poster", "formaction"]
    ...
    static func isPathContext(lineText: String, character: Int) -> String? {
    ...
    /// - Returns: 路径补全建议数组
    static func provideSuggestions(
        prefix: String,
        projectRoot: String,
