...
struct ColorPreviewView: View {
    ...
    init(color: Color, hexString: String, size: CGSize = CGSize(width: 12, height: 12)) {
        self.color = color
...
enum ColorParser {
    ...
    /// 匹配十六进制颜色
    static let hexPattern = try! NSRegularExpression(
        pattern: "#(?:[0-9a-fA-F]{3}){1,2}\\b",
        options: .caseInsensitive
    )

    /// 匹配 rgb/rgba 颜色
    static let rgbPattern = try! NSRegularExpression(
        pattern: "rgba?\\(\\s*\\d+\\s*,\\s*\\d+\\s*,\\s*\\d+\\s*(?:,\\s*[\\d.]+\\s*)?\\)",
        options: .caseInsensitive
    )

    /// 匹配 hsl/hsla 颜色
    static let hslPattern = try! NSRegularExpression(
    ...
    static func findColors(in text: String) -> [(range: NSRange, hexString: String, color: Color)] {
    ...
    /// 检查光标位置是否在颜色值附近
    static func isNearColor(text: String, character: Int, maxDistance: Int = 20) -> Color? {
