...
extension FinderSync {
    ...
    @IBAction func openInVSCode(_ sender: AnyObject?) {
        ...
        if Self.verbose {
            logInfo("待打开 URL 数量: \(urlsToOpen.count)")
        }
        if Self.verbose, let first = urlsToOpen.first {
            logInfo("首个 URL 路径: \(first.path)")
        }
    ...
    @IBAction func createNewFileFromTemplate(_ sender: AnyObject?) {
        if Self.verbose {
            logInfo("触发「从模板新建文件」操作")
        }
        guard let item = sender as? NSMenuItem else {
            if Self.verbose {
                logWarn("sender 不是 NSMenuItem 类型")
            }
            return
        }
        ...
        guard index >= 0, index < cachedTemplates.count else {
            if Self.verbose {
                logWarn("模板索引无效: \(index)，缓存数量: \(self.cachedTemplates.count)")
            }
            return
        }
        ...
        if Self.verbose {
            logInfo("创建文件 - 名称: \(template.name), 扩展名: \(template.extensionName)")
        }
    ...
    @IBAction func deleteFile(_ sender: AnyObject?) {
        ...
        guard alert.runModal() == .alertFirstButtonReturn else {
            if Self.verbose { logInfo("用户取消删除") }
            return
        }
        ...
        for url in items {
            do {
                ...
                if Self.verbose {
                    logInfo("已移至废纸篓: \(url.path)")
                }
            } catch {
                if Self.verbose {
                    logError("移至废纸篓失败: \(url.path)，错误: \(error.localizedDescription)")
                }
            }
        }
    }
    ...
    @IBAction func hideFile(_ sender: AnyObject?) {
        ...
        for url in items {
            do {
                ...
                if Self.verbose {
                    logInfo("已隐藏: \(url.path)")
                }
            } catch {
                if Self.verbose {
                    logError("隐藏失败: \(url.path)，错误: \(error.localizedDescription)")
                }
            }
        }
    }
    ...
    @IBAction func showHiddenFiles(_ sender: AnyObject?) {
        ...
        if let error = error {
            if Self.verbose {
                logError("执行 AppleScript 失败: \(error)")
            }
        } else {
            if Self.verbose {
                logInfo("成功显示隐藏文件")
