...
struct SwitchModelTool: LumiAgentTool, @unchecked Sendable {
    static let info = LumiAgentToolInfo(
        id: "switch_model",
        displayName: LumiPluginLocalization.string("Switch Model", bundle: .module),
        description: """
    ...
    func displayDescription(arguments: [String: LumiJSONValue]) -> String {
        LumiPluginLocalization.string("切换模型", bundle: .module)
    }
    ...
    func execute(arguments: [String: LumiJSONValue], context: LumiToolExecutionContext) async throws -> String {
        ...
        let providers = await MainActor.run { chatService.providerInfos }
        ...
        let previousProvider = await MainActor.run {
            chatService.providerID(for: conversationID) ?? chatService.selectedProviderID ?? "-"
        }
        let previousModel = await MainActor.run {
            chatService.modelName(for: conversationID) ?? chatService.selectedModel ?? "-"
        }
