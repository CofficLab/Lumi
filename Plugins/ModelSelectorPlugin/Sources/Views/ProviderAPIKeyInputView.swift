...
struct ProviderAPIKeyInputView: View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(alignment: .center, spacing: 6) {
                Group {
                    if isKeyVisible {
                        TextField(
                            LumiPluginLocalization.string("Enter API Key"),
                            text: $apiKey
                        )
                    } else {
                        SecureField(
            ...
            HStack(spacing: 12) {
                if didSave {
                    HStack(spacing: 4) {
                        ...
                        Text(
                            verbatim: LumiPluginLocalization.string("Saved")
                        )
                        .font(.system(size: 10))
                        .foregroundColor(theme.success)
                    }
                ...
                if supportsCancel {
                    Button(action: { onCancel?() }) {
                        Text(
                            verbatim: LumiPluginLocalization.string("Cancel")
                        )
    ...
    private var visibilityToggle: some View {
        ...
        .help(
            isKeyVisible
                ? LumiPluginLocalization.string("Hide API Key")
                : LumiPluginLocalization.string("Show API Key")
        )
    }
    ...
    private var saveButton: some View {
        Button(action: save) {
            Text(
                verbatim: LumiPluginLocalization.string("Save")
            )
