...
/// 若直接对每个 chunk 调用 `DeepSeekAnthropicEventParser.parse` / `DeepSeekEventParser.parse`
/// (两者都是无状态纯函数),跨 chunk 的帧——例如较大的 `content_block_delta`
/// (thinking / text)——会被切成两半后各自解析失败、事件整体丢失。
///
/// 丢失的 thinking/text 会让 Lumi 存储的 assistant 消息与模型实际输出不一致,
/// 下一轮请求回传历史消息时,`tool_use` 之前的 thinking blocks 与 DeepSeek
/// 落盘的「输出结束位置」缓存前缀单元失配 → 从第一条 assistant 消息起缓存全 miss,
/// 命中率崩到仅剩固定公共前缀(实测 ~2%)。
///
