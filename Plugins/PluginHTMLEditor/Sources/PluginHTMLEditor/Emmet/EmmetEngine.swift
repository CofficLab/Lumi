...
/// Emmet 缩写展开结果
public struct EmmetExpansion: Equatable, Sendable {
    /// 展开后的 HTML 文本
...
public enum EmmetEngine {
    ...
    public static func expand(_ abbreviation: String, syntax: EmmetSyntax = .html) -> EmmetExpansion? {
    ...
    public static func isValidAbbreviation(_ text: String) -> Bool {
...
public enum EmmetSyntax: Sendable {
    case html
    case xhtml
    case jsx

    public var selfClosingSuffix: String {
        switch self {
        case .html: return ""
...
/// Emmet 缩写解析后的抽象语法树节点
public struct EmmetNode: Equatable {
    /// 标签名
    ...
    /// 子节点
    public var children: [EmmetNode] = []
    /// 重复次数
    public var repeatCount: Int = 1
    /// 重复计数器变量 (如 `$` -> 1, 2, 3)
    public var hasCounter: Bool = false
    /// 相邻兄弟节点
    public var siblings: [EmmetNode] = []
...
private struct EmmetGenerator {
    public let syntax: EmmetSyntax
    private let indent: String = "  "

    public init(syntax: EmmetSyntax) {
        self.syntax = syntax
    }

    public func generate(from node: EmmetNode, abbreviation: String) -> EmmetExpansion {
