...
struct RemoteProviderSettingsPage: View {
    ...
    /// Provider instance resolved via LumiChatServicing protocol.
    private var selectedProviderInstance: (any LumiLLMProvider)? {
        chatService.provider(forID: selectedProviderID)
    }
    ...
    private var headerStats: some View {
        HStack(spacing: 10) {
            Label(
                String(localized: "\(remoteProviders.count) cloud providers"),
                systemImage: "cloud"
            )
            Text(verbatim: String(localized: "\(selectedProvider?.availableModels.count ?? 0) models"))
            Spacer()
        }
        .font(.appCaption)
            ...
            VStack(spacing: 10) {
                AppSearchBar(
                    ...
                    placeholder: LocalizedStringKey(
                        LumiPluginLocalization.string("Search providers", bundle: .module)
                    )
                )
            }
            .padding(12)
            ...
            ScrollView {
                LazyVStack(spacing: 4) {
                    ...
                    if filteredProviders.isEmpty {
                        AppEmptyState(icon: "magnifyingglass", title: "No providers found")
                            .padding(.vertical, 32)
                    }
                }
                .padding(8)
    ...
    private var providerDetailPane: some View {
        if let selectedProvider {
            ...
            AppEmptyState(icon: "cloud", title: "Select a provider")
                .frame(maxWidth: .infinity, maxHeight: .infinity)
                .appSurface(style: .panel, cornerRadius: 0)
        }
    }
    ...
    private var apiKeySection: some View {
        AppSettingsSection(title: "API Key", subtitle: "Configure your access credentials", spacing: 12) {
    ...
    private func modelSection(for provider: LumiLLMProviderInfo) -> some View {
        AppSettingsSection(title: "Available Models", spacing: 12) {
            VStack(spacing: 0) {
