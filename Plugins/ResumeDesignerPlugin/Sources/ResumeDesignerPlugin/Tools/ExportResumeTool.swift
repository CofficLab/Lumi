...
public struct ExportResumeTool: LumiAgentTool {
    public static let info = LumiAgentToolInfo(
        id: "resume_export",
        displayName: "Export resume",
        description: "Export one resume as vector PDF (print-ready, selectable text) and/or DPI-controlled PNG pages to an explicitly selected external directory."
    )
    public init() {}
    public var inputSchema: LumiJSONValue {
        ...
        properties["dpi"] = ["type": "integer", "enum": .array(ResumeExportResolution.allCases.map { LumiJSONValue.int($0.rawValue) }), "description": "PNG resolution. Defaults to 300 (print quality)."]
        ...
        return ["type": "object", "properties": .object(properties), "required": ["resumeId", "outputDirectory"]]
    }
    ...
    public func execute(arguments: [String: LumiJSONValue], kernel: KernelLumi) async throws -> String {
        ...
        let format = (arguments.string("format") ?? "pdf").lowercased()
        guard ["pdf", "png", "both"].contains(format) else {
            throw ResumeToolSupport.ToolArgumentError.invalid("format")
        }
        let dpiRaw = (arguments["dpi"]?.stringValue).flatMap(Int.init) ?? ResumeExportResolution.print.rawValue
        guard let resolution = ResumeExportResolution(rawValue: dpiRaw) else {
            throw ResumeToolSupport.ToolArgumentError.invalid("dpi")
        }
        ...
        guard ResumeDocumentStore.isPathAllowed(resolvedOutput, allowedDirectories: kernel.allowedDirectories) else {
            throw ResumeStoreError.pathNotAllowed(resolvedOutput)
        }
        ...
        if let pdfData {
            let destinationURL = outputDirectory.appendingPathComponent("\(resume.document.id).pdf")
