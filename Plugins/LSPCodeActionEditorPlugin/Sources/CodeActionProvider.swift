...
/// 为诊断问题提供快速修复建议（灯泡图标）
@MainActor
public final class CodeActionProvider: ObservableObject, SuperEditorCodeActionProvider {
    ...
    /// 当前可用的代码动作
    @Published public var actions: [CodeActionItem] = []
    /// 是否在请求中
    ...
    /// 请求代码动作
    public func requestCodeActions(uri: String, range: LSPRange, diagnostics: [Diagnostic]) async {
    ...
    /// 请求针对某一行诊断的代码动作
    public func requestCodeActionsForLine(
        uri: String,
        line: Int,
        character: Int,
