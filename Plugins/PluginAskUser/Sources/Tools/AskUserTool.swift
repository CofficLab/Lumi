...
public struct AskUserTool: SuperAgentTool, SuperLog {
    ...
    public func execute(arguments: [String: ToolArgument], context: ToolExecutionContext) async throws -> String {
        ...
        guard let question = arguments["question"]?.value as? String, !question.isEmpty else {
            return errorResult(message: "问题内容不能为空")
        }
        ...
        // 返回特殊标记 + JSON 数据
        // 渲染器通过 __ASK_USER_PENDING__ 标记识别
...
extension Notification.Name {
    /// 用户回答了 ask_user 工具的问题
    /// userInfo: ["toolCallId": String, "answer": String, "conversationId": String]
    public static let askUserDidRespond = Notification.Name("askUserDidRespond")
}

extension NotificationCenter {
    /// 发送用户回答通知
    public static func postAskUserResponse(
        toolCallId: String,
        answer: String,
        conversationId: String
    ) {
        NotificationCenter.default.post(
            name: .askUserDidRespond,
            object: nil,
            userInfo: [
