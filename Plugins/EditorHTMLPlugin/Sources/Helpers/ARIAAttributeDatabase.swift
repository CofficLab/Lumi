...
public enum ARIAAttributeDatabase {
    // MARK: - ARIA 角色

    public static let roles: [(name: String, summary: String)] = [
    ...
    public static let roleMap: [String: String] = Dictionary(
        uniqueKeysWithValues: roles.map { ($0.name.lowercased(), $0.summary) }
    )

    // MARK: - ARIA 属性

    public static let attributes: [(name: String, summary: String)] = [
    ...
    public static let attributeMap: [String: String] = Dictionary(
        uniqueKeysWithValues: attributes.map { ($0.name.lowercased(), $0.summary) }
    )

    // MARK: - 补全

    public static func roleSuggestions(prefix: String) -> [EditorCompletionSuggestion] {
    ...
    public static func ariaSuggestions(prefix: String) -> [EditorCompletionSuggestion] {
        let normalized = prefix.lowercased()
        return attributes
            .filter { normalized.isEmpty || $0.name.hasPrefix(normalized) }
            .map { attr in
