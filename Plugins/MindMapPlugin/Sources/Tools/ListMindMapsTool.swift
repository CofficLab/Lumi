...
/// 列出指定作用域下的全部思维导图。
public struct ListMindMapsTool: LumiAgentTool {
    public static let info = LumiAgentToolInfo(
        id: "list_mind_maps",
        displayName: "List Mind Maps",
        description: "List all mind maps in a storage scope (project or app). Returns id, title, node count and updatedAt for each."
    )
    ...
    public func execute(arguments: [String: LumiJSONValue], kernel: KernelLumi) async throws -> String {
        ...
        let includeBoth = arguments.bool("includeBothScopes") ?? false
    ...
    private func formatSingleScope(scope: MindMapScope, maps: [MindMap], language: LumiLanguagePreference) -> String {
        switch language {
        case .chinese:
    ...
    private func formatTwoScopes(project: [MindMap], app: [MindMap], language: LumiLanguagePreference) -> String {
