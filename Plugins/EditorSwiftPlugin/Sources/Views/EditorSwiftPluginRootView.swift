...
private enum EditorXcodeProjectPreloader {
    ...
    private static func fetchAvailableSchemes(for workspaceURL: URL) async -> [String] {
        ...
        if SwiftPluginLog.verbose {
            if SwiftPluginLog.verbose {
                SwiftPluginLog.logger.info("\(logPrefix)开始获取 schemes：xcodebuild \(args.joined(separator: " "))")
            }
        }
        ...
        return await withCheckedContinuation { continuation in
            ...
            process.terminationHandler = { _ in
                guard process.terminationStatus == 0,
                    ...
                    if SwiftPluginLog.verbose {
                        if SwiftPluginLog.verbose {
                            SwiftPluginLog.logger.warning("\(logPrefix)xcodebuild 获取 schemes 失败，terminationStatus=\(process.terminationStatus)")
                        }
                    }
                ...
                if SwiftPluginLog.verbose {
                    if SwiftPluginLog.verbose {
                        SwiftPluginLog.logger.info("\(logPrefix)xcodebuild 获取 schemes 完成，count=\(uniqueSchemes.count)，schemes=\(uniqueSchemes.joined(separator: ", "))")
                    }
                }
            ...
            do {
                ...
                if SwiftPluginLog.verbose {
                    if SwiftPluginLog.verbose {
                        SwiftPluginLog.logger.error("\(logPrefix)xcodebuild 启动失败：\(error.localizedDescription)")
                    }
                }
                continuation.resume(returning: [])
            }
