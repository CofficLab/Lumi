...
public enum GitCommitDetailService {
    ...
    public static func fileIcon(for file: String) -> String {
    ...
    /// 根据文件扩展名返回图标颜色
    public static func fileIconColor(for file: String) -> Color {
        let ext = (file as NSString).pathExtension.lowercased()
        switch ext {
    ...
    public static func copyHash(_ hash: String) {
        #if os(macOS)
    ...
    public static func loadCommitDetail(path: String, hash: String) async throws -> (GitCommitDetail, [GitChangedFile]) {
    ...
    /// - Parameter path: 项目路径
    /// - Returns: 变更文件列表
    public static func loadUncommittedFiles(path: String) async throws -> [GitChangedFile] {
    ...
    public struct ProjectGitInfo {
        ...
        public let lastCommitDate: String
    }

    /// 加载项目 Git 概览信息（工作区干净时显示）
    public static func loadProjectGitInfo(path: String) async -> ProjectGitInfo? {
        // 并发执行多个 git 命令
    ...
    /// - Returns: (旧文本, 新文本)
    public static func loadFileDiff(file: String, projectPath: String, commitHash: String?) async throws -> (String, String) {
