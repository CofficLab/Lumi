...
public final class AliyunProvider: NSObject, SuperLLMProvider, SuperLog, @unchecked Sendable {
    ...
    public static let displayName = String(localized: "阿里云 CodingPlan", bundle: .module)
    public static let shortName = "Aliyun"
    public static let description = String(localized: "阿里云 DashScope Coding Plan（兼容 Anthropic API）", bundle: .module)

    public static let websiteURL: String? = "https://dashscope.console.aliyun.com"

    // MARK: - 配置相关

    public static let apiKeyStorageKey = "DevAssistant_ApiKey_Aliyun"
    public static let defaultModel = "qwen3.6-plus"
        ...
        do {
            ...
            if let error = json?["error"] as? [String: Any],
               let errorMessage = error["message"] as? String {
                return LumiCoreKit.StreamChunk(error: errorMessage, eventType: .unknown, rawEvent: text)
            }

            // 处理 ping 事件
            if effectiveEventType == "ping" {
                return LumiCoreKit.StreamChunk(eventType: .ping, rawEvent: text)
            }
            ...
            if effectiveEventType == "message_start" {
                ...
                return LumiCoreKit.StreamChunk(eventType: .messageStart, rawEvent: text, inputTokens: inputTokens)
            }
            ...
            if effectiveEventType == "message_delta" {
                ...
                return LumiCoreKit.StreamChunk(eventType: .messageDelta, rawEvent: text, inputTokens: inputTokens, outputTokens: outputTokens, stopReason: stopReason)
            }

            // 处理消息结束
            if effectiveEventType == "message_stop" {
                return LumiCoreKit.StreamChunk(isDone: true, eventType: .messageStop, rawEvent: text)
            }
                ...
                if let contentBlock = json?["content_block"] as? [String: Any],
                   let blockType = contentBlock["type"] as? String {
                    ...
                    if blockType == "thinking" {
                        return LumiCoreKit.StreamChunk(eventType: .contentBlockStart, rawEvent: text)
                    }
                    ...
                    if blockType == "text" {
                        if let textContent = contentBlock["text"] as? String, !textContent.isEmpty {
                            return LumiCoreKit.StreamChunk(content: textContent, eventType: .contentBlockStart, rawEvent: text)
                        }
                        return LumiCoreKit.StreamChunk(eventType: .contentBlockStart, rawEvent: text)
                    }
            ...
            if effectiveEventType == "content_block_delta" {
                if let delta = json?["delta"] as? [String: Any] {
                    if let thinkingDelta = delta["thinking_delta"] as? String {
                        return LumiCoreKit.StreamChunk(content: thinkingDelta, eventType: .thinkingDelta, rawEvent: text)
                    }
                    ...
                    if let textContent = delta["text"] as? String {
                        return LumiCoreKit.StreamChunk(content: textContent, eventType: .textDelta, rawEvent: text)
                    }

                    if let textDelta = delta["text_delta"] as? String {
                        return LumiCoreKit.StreamChunk(content: textDelta, eventType: .textDelta, rawEvent: text)
                    }

                    if let partialJson = delta["partial_json"] as? String {
                        return LumiCoreKit.StreamChunk(partialJson: partialJson, eventType: .inputJsonDelta, rawEvent: text)
                    }

                    if delta["signature"] != nil {
                        return LumiCoreKit.StreamChunk(eventType: .signatureDelta, rawEvent: text)
                    }

                    return LumiCoreKit.StreamChunk(eventType: .contentBlockDelta, rawEvent: text)
                }
                return LumiCoreKit.StreamChunk(eventType: .contentBlockDelta, rawEvent: text)
            }
            ...
            if effectiveEventType == "content_block_stop" {
                return LumiCoreKit.StreamChunk(eventType: .contentBlockStop, rawEvent: text)
            }

            return LumiCoreKit.StreamChunk(eventType: .unknown, rawEvent: text)
        } catch {
            if Self.verbose {
                Self.logger.warning("\(self.t)解析流式数据块失败: \(error.localizedDescription)")
            }
            return LumiCoreKit.StreamChunk(error: "解析失败: \(error.localizedDescription)", eventType: .unknown, rawEvent: text)
        }
    }
}
