...
import SwiftUI
import Testing
@testable import EditorHostPlugin
...
struct EditorHostPluginTests {
    ...
    func onBootRegistersBothContracts() async throws {
        let kernel = KernelLumi()

        let plugin = EditorHostPlugin()
        try await plugin.onBoot(kernel: kernel)

        // legacy 契约（PluginManager 编辑器插件装配依赖）。
        #expect(kernel.editorProvider != nil)
        // V2 契约（kernel.editorV2），Surface 由 Host 注入。
        let editorV2 = try #require(kernel.editorV2)
        #expect(editorV2.documents != nil)
        #expect(editorV2.sessions.state.groups.count == 1)

        let surfaceView = editorV2.surface.makeEditorView()
        #expect(surfaceView is AnyView)
    }
