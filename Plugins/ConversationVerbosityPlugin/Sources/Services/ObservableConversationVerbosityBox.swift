...
/// SwiftUI 友好的 `ConversationManaging`（仅 verbosity 相关）窄播包装器。
///
/// 与 `ObservableMessageStreamingBox` 同理：SwiftUI 的 `@ObservedObject` 不支持
/// `any ConversationManaging` 这样的存在类型，需要用具体类包装，把 service 的
/// `objectWillChange` 桥接到自己的 publisher 上。
///
/// **为何需要独立订阅**：本插件只关心 `globalVerbosity` 一个值。若经 kernel 的全局
/// objectWillChange 转发，project/workspace/settings 等无关服务变更都会让本工具栏
...
/// ```
@MainActor
final class ObservableConversationVerbosityBox: ObservableObject {
    /// 被包装的服务实例。绑定前为 nil。
    private(set) var service: (any ConversationManaging)?
    ...
    func bind(_ service: (any ConversationManaging)?) {
        // 已绑定同一实例：跳过，避免重复订阅。
