...
enum MiniMaxResponsesEventParser {
    static func parse(_ data: Data) -> [MiniMaxResponsesEvent] {
        ...
        return text.components(separatedBy: "\n\n").compactMap { frame -> MiniMaxResponsesEvent? in
            ...
            guard let dataLine = lines.first(where: { $0.hasPrefix("data:") }) else {
                if eventName == "[DONE]" || eventName == "done" {
                    return doneEvent()
                }
                return nil
            }
            ...
            let eventType = eventName ?? json["type"] as? String

            if eventType == "error" || eventType == "response.failed" {
                let response = json["response"] as? [String: Any]
                let error = json["error"] as? [String: Any] ?? response?["error"] as? [String: Any]
                return errorEvent(error?["message"] as? String)
            }

            // MiniMax currently follows the Responses API event names and puts
            // text/reasoning deltas directly in the top-level `delta` string.
            switch eventType {
            case "response.output_text.delta":
                return textEvent(json["delta"] as? String)
            case "response.reasoning_text.delta":
                return reasoningEvent(json["delta"] as? String)
            ...
            case "response.function_call_arguments.delta":
                return functionCallEvent(
                    callID: nil,
                    name: nil,
                    arguments: json["delta"] as? String
                )
            case "response.completed":
                let response = json["response"] as? [String: Any]
                return doneEvent(
                    stopReason: response?["status"] as? String,
                    usage: usage(from: response?["usage"] as? [String: Any])
                )
            default:
                break
            }

            // Keep compatibility with the earlier delta-object representation.
            if let delta = json["delta"] as? [String: Any] {
                switch delta["type"] as? String {
                case "text_delta":
                    return textEvent(delta["text"] as? String)
                case "reasoning_delta":
                    return reasoningEvent(delta["text"] as? String)
                case "function_call":
                    return functionCallEvent(
                        callID: delta["call_id"] as? String,
            ...
            if eventType == "done" {
                return doneEvent(
                    ...
                    usage: usage(from: json["usage"] as? [String: Any])
                )
            }
    ...
    private static func textEvent(_ text: String?) -> MiniMaxResponsesEvent {
        MiniMaxResponsesEvent(type: .textDelta, text: text, reasoning: nil, callID: nil, name: nil, arguments: nil, stopReason: nil, isDone: false, error: nil, usage: nil)
    }

    private static func reasoningEvent(_ reasoning: String?) -> MiniMaxResponsesEvent {
        MiniMaxResponsesEvent(type: .reasoningDelta, text: nil, reasoning: reasoning, callID: nil, name: nil, arguments: nil, stopReason: nil, isDone: false, error: nil, usage: nil)
    }

    private static func functionCallEvent(callID: String?, name: String?, arguments: String?) -> MiniMaxResponsesEvent {
        MiniMaxResponsesEvent(type: .functionCall, text: nil, reasoning: nil, callID: callID, name: name, arguments: arguments, stopReason: nil, isDone: false, error: nil, usage: nil)
    }

    private static func doneEvent(
        stopReason: String? = nil,
        usage: MiniMaxResponsesUsage? = nil
    ) -> MiniMaxResponsesEvent {
        MiniMaxResponsesEvent(type: .done, text: nil, reasoning: nil, callID: nil, name: nil, arguments: nil, stopReason: stopReason, isDone: true, error: nil, usage: usage)
    }

    private static func errorEvent(_ message: String?) -> MiniMaxResponsesEvent {
        MiniMaxResponsesEvent(type: .error, text: nil, reasoning: nil, callID: nil, name: nil, arguments: nil, stopReason: nil, isDone: false, error: message, usage: nil)
    }

    private static func usage(from dictionary: [String: Any]?) -> MiniMaxResponsesUsage? {
        guard let dictionary else { return nil }
        return MiniMaxResponsesUsage(
            inputTokens: dictionary["input_tokens"] as? Int ?? 0,
            outputTokens: dictionary["output_tokens"] as? Int ?? 0,
            totalTokens: dictionary["total_tokens"] as? Int ?? 0,
