...
final class MiniMaxMessageState: @unchecked Sendable {
    private let lock = NSLock(); private var content = ""; private var reasoning = ""; private var calls: [LumiToolCall] = []; private var activeID: String?; private var activeName: String?; private var activeArgs = ""; private var error: String?; private var stopReason: String?; private var started: Date; private var first: Date?; private var ended: Date?; private let conversationID: UUID; private let providerID: String; private let model: String; private var input: Int?; private var output: Int?
    ...
    func append(_ event: MiniMaxOpenAIEvent) { lock.lock(); defer { lock.unlock() }; if let value = event.content, !value.isEmpty { if first == nil { first = Date() }; content += value }; if let value = event.reasoning, !value.isEmpty { if first == nil { first = Date() }; reasoning += value }; input = event.inputTokens ?? input; output = event.outputTokens ?? output; stopReason = event.stopReason ?? stopReason; for delta in event.toolDeltas { if delta.id != nil || delta.name != nil { saveToolLocked(); activeID = delta.id ?? UUID().uuidString; activeName = delta.name ?? ""; activeArgs = delta.arguments } else { activeArgs += delta.arguments } } }
    ...
    func finish() { lock.lock(); ended = Date(); saveToolLocked(); lock.unlock() }
