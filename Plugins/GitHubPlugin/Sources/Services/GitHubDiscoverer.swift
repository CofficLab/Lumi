...
public struct GitHubInsightDiscoverer: Sendable, SuperLog {
    ...
    public func discover(profile: GitHubInsightProjectProfile, limitPerQuery: Int = 8) async throws -> [GitHubInsightKBEntry] {
        ...
        GitHubPlugin.logger.info("\(Self.t)开始 GitHub 发现：project=\(profile.projectPath)，queries=\(searchPlans.count)，limitPerQuery=\(limitPerQuery)")
        ...
        for (index, plan) in searchPlans.enumerated() {
            ...
            for (attemptIndex, attempt) in plan.attempts.enumerated() {
                GitHubPlugin.logger.info("\(Self.t)请求 GitHub 仓库搜索[\(index + 1)/\(searchPlans.count)]：name=\(plan.name)，stage=\(attempt.stage)，query=\(attempt.query)")
                ...
                do {
                    ...
                    if entriesByRepo.isEmpty {
                        GitHubPlugin.logger.error("\(Self.t)GitHub 搜索失败且暂无可用候选[\(index + 1)/\(searchPlans.count)]：name=\(plan.name)，stage=\(attempt.stage)，错误=\(error.localizedDescription)")
                        throw error
                    }

                    let entries = finalizedEntries(from: entriesByRepo)
                    GitHubPlugin.logger.warning("\(Self.t)GitHub 搜索中断，返回已发现的部分结果[\(index + 1)/\(searchPlans.count)]：name=\(plan.name)，stage=\(attempt.stage)，错误=\(error.localizedDescription)，候选唯一仓库=\(entriesByRepo.count)，输出条目=\(entries.count)")
                    return entries
                }
                ...
                GitHubPlugin.logger.info("\(Self.t)GitHub 搜索完成[\(index + 1)/\(searchPlans.count)]：stage=\(attempt.stage)，total=\(result.totalCount)，返回=\(result.items.count)，接受=\(acceptedCount)，归档过滤=\(archivedCount)，低信号 fork 过滤=\(lowSignalForkCount)，已有依赖过滤=\(existingDependencyCount)，累计唯一仓库=\(entriesByRepo.count)")
                ...
                if attemptIndex < plan.attempts.count - 1 {
                    GitHubPlugin.logger.info("\(Self.t)GitHub 搜索无可用结果，准备放宽查询[\(index + 1)/\(searchPlans.count)]：name=\(plan.name)，nextStage=\(plan.attempts[attemptIndex + 1].stage)")
                }
            }

            if !didAcceptRepository {
                GitHubPlugin.logger.info("\(Self.t)GitHub 搜索计划未找到可用仓库[\(index + 1)/\(searchPlans.count)]：name=\(plan.name)")
            }
        }

        let entries = finalizedEntries(from: entriesByRepo)

        GitHubPlugin.logger.info("\(Self.t)GitHub 发现结束：候选唯一仓库=\(entriesByRepo.count)，输出条目=\(entries.count)")
        return entries
    }
