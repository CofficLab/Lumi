...
enum ToolExecutorRuntimeBridge {
    ...
    static var presentToolPermissionIfNeeded: (AgentChatMessage, UUID) async -> Bool = { _, _ in false }
    static var executeToolCalls: (AgentChatMessage, UUID) async -> ToolExecutionSummary = { _, _ in ToolExecutionSummary() }
