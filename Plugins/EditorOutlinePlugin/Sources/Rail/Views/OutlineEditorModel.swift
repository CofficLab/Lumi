...
/// Outline 视图模型：订阅 KernelLumi V2 符号/选择/文档状态并暴露操作。
@MainActor
public final class OutlineEditorModel: ObservableObject {
    ...
    /// 光标所在符号路径（根到最深节点，保留顺序）。
    public func activePath(for line: Int) -> [EditorDocumentSymbol] {
        symbols.compactMap { $0.activePath(for: line) }
            .max(by: { $0.count < $1.count }) ?? []
    }

    public func activePathIDs(for line: Int) -> Set<String> {
        Set(activePath(for: line).map(\.id))
    }

    public func activeAncestorIDs(for line: Int) -> Set<String> {
        Set(activePath(for: line).dropLast().map(\.id))
    }
    ...
    /// 请求刷新活动文档符号。
    public func refresh() {
        editor.documentSymbols.refresh()
    }
}
