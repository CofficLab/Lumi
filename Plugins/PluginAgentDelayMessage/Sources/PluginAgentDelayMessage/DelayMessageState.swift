...
@MainActor
public final class DelayMessageState: ObservableObject {
    ...
    public func enqueueDelayedMessage(conversationId: UUID, content: String) {
        guard let enqueueHandler else {
            DelayMessagePlugin.logger.error("⏳ DelayMessageState: enqueueHandler 不可用")
            return
        }

        enqueueHandler(conversationId, content)

        DelayMessagePlugin.logger.info("⏳ 已入队延时消息到会话 \(conversationId.uuidString.prefix(8))")
    }
}
