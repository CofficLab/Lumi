...
public final class SublyxPlugin: LumiPlugin {
    ...
    public func onBoot(kernel: KernelLumi) async throws {
        if let storage = kernel.storage {
            AvailabilityDiskCacheDirectoryResolver.set(
                pluginName: "LLMProviderSublyxPlugin",
                directory: storage.pluginDataDirectory(for: "LLMProviderSublyxPlugin")
            )
        }
    }
    ...
    public func pluginAboutView(kernel: KernelLumi) -> AnyView? {
        AnyView(
            LLMProviderLandingPage(displayName: LumiPluginLocalization.string("Sublyx", bundle: .module), icon: "link")
        )
    }
