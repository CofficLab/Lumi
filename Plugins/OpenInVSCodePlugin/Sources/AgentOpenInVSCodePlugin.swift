...
/// 在 VSCode 中打开项目插件
///
/// 在状态栏添加图标，点击后在 Visual Studio Code 编辑器中打开当前项目。当前项目路径由内核的
/// `ProjectProviding` 提供（响应式）。
@MainActor
public final class AgentOpenInVSCodePlugin: LumiPlugin {
    public let id = "com.coffic.lumi.plugin.open-in-vscode"
    public var name: String {
        LumiPluginLocalization.string("Open in VSCode", bundle: .module)
    }
    public let order = 63
    ...
    public func statusBarItems(kernel: KernelLumi) -> [StatusBarItem] {
        ...
        return [
            StatusBarItem(
                ...
                placement: .leading,
                statusBarView: {
                    OpenInVSCodeStatusBarView(project: project)
                }
            ),
        ]
    }
...
private enum VSCodeOpener {
    static func open(_ url: URL) {
        let workspace = NSWorkspace.shared
        guard let appURL = workspace.urlForApplication(withBundleIdentifier: "com.microsoft.VSCode")
...
/// VSCode 打开状态栏视图
public struct OpenInVSCodeStatusBarView: View {
    ...
    private var hasProjectView: some View {
        StatusBarHoverContainer(
            detailView: OpenInVSCodeDetailView(path: currentProjectPath),
            id: "open-in-vscode-status"
        ) {
            Button(action: {
                openInVSCode()
            }) {
            ...
            .buttonStyle(.plain)
            .help(LumiPluginLocalization.string("在 VSCode 中打开当前项目", bundle: .module))
        }
    }
    ...
    private var emptyView: some View {
        HStack(spacing: 6) {
            ...
            Text(LumiPluginLocalization.string("VSCode", bundle: .module))
                .font(.appMicro)
        }
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
    ...
    private func openInVSCode() {
        ...
        VSCodeOpener.open(url)
    }
}

// MARK: - Detail View

/// VSCode 打开详情视图（在 popover 中显示）
public struct OpenInVSCodeDetailView: View {
    @LumiTheme private var theme: any LumiUITheme
    let path: String
