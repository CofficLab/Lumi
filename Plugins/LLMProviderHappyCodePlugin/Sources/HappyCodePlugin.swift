...
public final class HappyCodePlugin: LumiPlugin {
    ...
    public func onBoot(kernel: KernelLumi) async throws {
        if let storage = kernel.storage {
            AvailabilityDiskCacheDirectoryResolver.set(
                pluginName: "LLMProviderHappyCodePlugin",
                directory: storage.pluginDataDirectory(for: "LLMProviderHappyCodePlugin")
            )
        }
    }
    ...
    public func pluginAboutView(kernel: KernelLumi) -> AnyView? {
        AnyView(
            LLMProviderLandingPage(displayName: LumiPluginLocalization.string("HappyCode", bundle: .module), icon: "face.smiling")
        )
    }
