...
public final class HyperAPIPlugin: LumiPlugin {
    ...
    public func onBoot(kernel: KernelLumi) async throws {
        if let storage = kernel.storage {
            AvailabilityDiskCacheDirectoryResolver.set(
                pluginName: "LLMProviderHyperAPIPlugin",
                directory: storage.pluginDataDirectory(for: "LLMProviderHyperAPIPlugin")
            )
        }
    }
    ...
    public func pluginAboutView(kernel: KernelLumi) -> AnyView? {
        AnyView(
            LLMProviderLandingPage(displayName: LumiPluginLocalization.string("HyperAPI", bundle: .module), icon: "bolt.horizontal")
        )
    }
