...
APP_ICON_CHECKSUM_BEFORE="$(shasum -a 256 "${TEST_ROOT}/repository/AppIconDesignerApp/AppIconDesigner.xcconfig")"
(
  cd "${TEST_ROOT}/repository/ci_scripts"
  CI_TAG=v9.9.9 CI_BUILD_NUMBER=32 ./ci_post_clone.sh
)
APP_ICON_CHECKSUM_AFTER="$(shasum -a 256 "${TEST_ROOT}/repository/AppIconDesignerApp/AppIconDesigner.xcconfig")"
if [[ "${APP_ICON_CHECKSUM_BEFORE}" != "${APP_ICON_CHECKSUM_AFTER}" ]]; then
  echo "FAIL: 非 app 发布 tag 不应修改 AppIconDesigner 版本" >&2
  exit 1
fi

if (
  cd "${TEST_ROOT}/repository/ci_scripts"
  CI_TAG=appicondesigner-v6.0.0 ./ci_post_clone.sh
) >/dev/null 2>&1; then
