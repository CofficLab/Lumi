...
/// 通过 DocsViewProviding 的 manual 条目暴露,在 设置 → 通用 → 新手引导 → 说明书 中阅读。
struct DesignerManualView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            ManualHeader(
                title: L.string("Resume Designer"),
                subtitle: L.string("User Manual")
            )

            ManualSectionHeader(number: 1, title: L.string("Overview"))
            Text(L.string("This manual covers the interface and basic operations of the Resume Designer: asking the Agent to create resumes, previewing and editing them, and exporting or printing the result."))
                .font(.appBody)
            ...
            ManualSectionHeader(number: 3, title: L.string("Basic Operations"))
            ManualStepList(items: [
                .init(L.string("Ask the Agent in the chat to create a resume, e.g. \"help me write a resume\".")),
                .init(L.string("Select a resume in the Resumes list to open it.")),
                .init(L.string("Switch between Preview and Source in the toolbar to view the page or the HTML source.")),
                .init(L.string("In Preview, click a block to draft an edit request for the Agent.")),
                .init(L.string("Click Export PDF or Export PNG and choose a folder, or click Print.")),
            ])
    ...
    private var interfaceFigure: some View {
        ManualFigure(caption: L.string("Figure 1: Interface layout")) {
            VStack(spacing: 12) {
                HStack(spacing: 0) {
                    // ① 简历列表示意
                    VStack(alignment: .leading, spacing: 7) {
                        groupLabel(L.string("Resumes"))
                ...
                HStack(spacing: 16) {
                    ManualFigureLegendItem(1, L.string("Resumes"))
                    ManualFigureLegendItem(2, L.string("Toolbar"))
                    ManualFigureLegendItem(3, L.string("Preview"))
                }
            }
        }
    }
