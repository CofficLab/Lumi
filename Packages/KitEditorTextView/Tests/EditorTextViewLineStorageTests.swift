...
/// Tests for the red-black tree backing the layout manager's line storage.
/// Ported from upstream CodeEditTextView's TextLayoutLineStorageTests.
final class EditorTextViewLineStorageTests: XCTestCase {
    ...
    func test_getLineQueries() throws {
        let tree = createBalancedTree()

        // By offset
        XCTAssertEqual(tree.getLine(atOffset: 0)?.index, 0)
        XCTAssertEqual(tree.getLine(atOffset: 1)?.index, 1)
        XCTAssertEqual(tree.getLine(atOffset: tree.length - 1)?.index, 14)
        XCTAssertNil(tree.getLine(atOffset: tree.length + 100))

        // By index
        XCTAssertEqual(tree.getLine(atIndex: 0)?.range, NSRange(location: 0, length: 1))
        XCTAssertEqual(tree.getLine(atIndex: 14)?.range.location, 105)
        XCTAssertNil(tree.getLine(atIndex: 15))

        // By y position
        XCTAssertEqual(tree.getLine(atPosition: 0.5)?.index, 0)
        XCTAssertEqual(tree.getLine(atPosition: 3.2)?.index, 3)
        // Positions at or past the height resolve to the last line
        XCTAssertEqual(tree.getLine(atPosition: 100)?.index, 14)

        XCTAssertEqual(tree.first?.index, 0)
        XCTAssertEqual(tree.last?.index, 14)
    }

    func test_buildFromTextStorageSplitsLines() {
        ...
        XCTAssertEqual(lineStorage.count, 3)
        XCTAssertEqual(lineStorage.length, 13)
        XCTAssertEqual(lineStorage.height, 30, accuracy: 0.05)
        XCTAssertEqual(lineStorage.first?.range, NSRange(location: 0, length: 4))
        XCTAssertEqual(lineStorage.last?.range, NSRange(location: 8, length: 5))
    }
