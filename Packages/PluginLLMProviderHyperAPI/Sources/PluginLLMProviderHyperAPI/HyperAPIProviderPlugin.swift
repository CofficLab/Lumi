...
/// HyperAPI 供应商装配插件（KernelCore 生态）。
///
/// 在 `onBoot` 中把本供应商的 HyperAPIProvider 注册进
/// `LLMProviderManagerProviding`，聊天链路即可经管理器路由到该供应商。
@MainActor
public final class HyperAPIProviderPlugin: SuperPlugin, SuperLog {
    nonisolated static let logger = Logger(subsystem: "com.coffic.lumi.plugin.llm-provider.hyperapi", category: "HyperAPI")
    ...
    public func onBoot(kernel: KernelCoreContainer) throws {
        ...
        let providers: [any SuperLLMProvider] = [HyperAPIProvider(networkProvider: networkProvider)]
