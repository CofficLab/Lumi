...
struct PluginWebSearchTests {
    ...
    func pluginMetadata() {
        ...
        #expect(WebSearchPlugin().name.isEmpty == false)
        #expect(WebSearchPlugin().metadata.category == .integration)
        #expect(WebSearchPlugin().order == 101)
        #expect(WebSearchPlugin().metadata.policy == .enabledByDefault)
    }

    @Test("plugin registers one web search tool")
    func pluginRegistersTool() {
        #expect(WebSearchPlugin.agentTools.count == 1)
        #expect(WebSearchPlugin.agentTools.first?.name == "web_search")
    }
    ...
    func toolSchemaRequiresQuery() throws {
        ...
        #expect((properties["query"] as? [String: String])?["type"] == "string")
    }
    ...
    func toolTrimsCopiedQueryWhitespace() async throws {
        ...
        let result = try await tool.execute(arguments: ["query": ToolArgument(" \nLumi release notes\t")])
    ...
    func toolReportsEmptySearchResult() async throws {
        ...
        let result = try await tool.execute(arguments: ["query": ToolArgument("unknown term")])
