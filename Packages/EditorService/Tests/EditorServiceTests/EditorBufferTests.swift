...
final class EditorBufferTests: XCTestCase {
    ...
    func testApplyEmptyTransactionPreservesSnapshotButPassesSelectionsThrough() {
        let buffer = EditorBuffer(text: "abc", version: 7)
        let transaction = EditorTransaction(
            replacements: [],
            updatedSelections: [
        ...
        XCTAssertEqual(result?.snapshot.text, "abc")
        XCTAssertEqual(result?.snapshot.version, 7)
        XCTAssertEqual(result?.selections?.first?.range, EditorRange(location: 2, length: 0))
