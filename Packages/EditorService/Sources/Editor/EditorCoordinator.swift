...
final class ContextMenuManager: SuperLog {
    ...
    func register(textView: TextView, state: EditorState) {
        ...
        if objc_getAssociatedObject(targetClass, Self.swizzledKey) == nil {
            ...
            if Self.verbose { editorContextMenuLog.info("\(self.t)首次 swizzle class=\(String(describing: targetClass))") }
    ...
    private func swizzleMenuForClass(_ targetClass: AnyClass) {
        ...
        guard let originalMethod = class_getInstanceMethod(targetClass, selector) else {
            if Self.verbose { editorContextMenuLog.warning("\(self.t)swizzleMenuForClass: 找不到 menu(for:) 方法, class=\(String(describing: targetClass))") }
            return
        }
        ...
        if Self.verbose { editorContextMenuLog.info("\(self.t)swizzleMenuForClass: 替换成功, class=\(String(describing: targetClass))") }
    }
}
...
final class ContextMenuHelper: NSObject, SuperLog {
    ...
    func injectCustomItems(into menu: NSMenu) {
        ...
        guard let textView, let state else {
            if Self.verbose { editorContextMenuLog.warning("\(self.t)injectCustomItems: textView=\(self.textView != nil) state=\(self.state != nil), 跳过注入") }
            return
        }
