...
@MainActor
public final class LSPService: ObservableObject, SuperLog {
    
    /// 插件专用 Logger
    public nonisolated static let logger = Logger(subsystem: "com.coffic.lumi", category: "lsp.service")
    ...
    @Published public var currentDiagnostics: [Diagnostic] = []
    @Published public var isAvailable: Bool = false
    @Published public var activeLanguageId: String?
    ...
    public func startServer(for languageId: String, projectPath: String) async {
    ...
    public static func makeWorkspaceFolders(for languageId: String, projectPath: String) -> [LanguageServerProtocol.WorkspaceFolder]? {
    ...
    public static func makeInitializationOptions(for languageId: String, projectPath: String) -> LanguageServerProtocol.LSPAny? {
        guard let capability = languageIntegrationCapability(for: languageId, projectPath: projectPath),
    ...
    public func setProjectRootPath(_ projectPath: String?) {
        projectRootPath = projectPath
        if Self.verbose {
            if Self.verbose {
    ...
    public func openDocument(uri: String, languageId: String, text: String, version: Int = 0) async {
    ...
    public func closeDocument(uri: String) {
        let server = server
    ...
    public func updateDocumentSnapshot(uri: String, text: String) {
        guard uri == currentURI else { return }
        latestDocumentSnapshot = text
    }
    
    public func documentDidChange(uri: String, range: LSPRange, text: String, version: Int) {
        guard uri == currentURI else { return }
        currentVersion = version
    ...
    public func replaceDocument(uri: String, text: String, version: Int) {
        guard uri == currentURI else { return }
        currentVersion = version
        latestDocumentSnapshot = text
        pendingChanges.removeAll()
    ...
    public func requestHover(uri: String, line: Int, character: Int) async -> String? {
    ...
    public func requestHoverRaw(uri: String, line: Int, character: Int) async -> Hover? {
        guard let server else { return nil }
        do {
    ...
    public func requestDefinition(uri: String, line: Int, character: Int) async -> Location? {
        guard let server else { return nil }
    ...
    public func requestReferences(uri: String, line: Int, character: Int) async -> [Location] {
        guard let server else { return [] }
    ...
    public func requestDocumentSymbols(uri: String) async -> [DocumentSymbol] {
        guard let server else { return [] }
        do {
    ...
    public func requestRename(uri: String, line: Int, character: Int, newName: String) async -> WorkspaceEdit? {
        guard let server else { return nil }
    ...
    public func requestFormatting(uri: String, tabSize: Int, insertSpaces: Bool) async -> [TextEdit]? {
        guard let server else { return nil }
        do {
    ...
    public func requestSignatureHelp(uri: String, line: Int, character: Int) async -> SignatureHelp? {
        guard let server else { return nil }
    ...
    public func requestInlayHint(uri: String, startLine: Int, startCharacter: Int, endLine: Int, endCharacter: Int) async -> [InlayHint]? {
        guard let server else { return nil }
    ...
    public func requestDocumentHighlight(uri: String, line: Int, character: Int) async -> [DocumentHighlight] {
        guard let server else { return [] }
    ...
    public func requestCodeAction(uri: String, range: LSPRange, diagnostics: [Diagnostic], triggerKinds: [CodeActionKind]? = nil) async -> [CodeAction] {
        guard let server else { return [] }
    ...
    public var codeActionResolveSupported: Bool {
    ...
    public var supportsInlayHints: Bool {
        guard let server else { return false }
        return server.capabilities.inlayHintProvider != nil
    }
    
    public func requestSemanticTokens(uri: String) async -> SemanticTokens? {
        guard let server else { return nil }
        do {
    ...
    public func requestSemanticTokensDelta(uri: String, previousResultId: String) async -> SemanticTokensDeltaResponse? {
        guard let server else { return nil }
        do {
    ...
    public func requestDeclaration(uri: String, line: Int, character: Int) async -> DeclarationResponse {
        guard let server else { return nil }
    ...
    public func requestTypeDefinition(uri: String, line: Int, character: Int) async -> TypeDefinitionResponse {
        guard let server else { return nil }
    ...
    public func requestImplementation(uri: String, line: Int, character: Int) async -> ImplementationResponse {
        guard let server else { return nil }
    ...
    public var currentSemanticTokenMap: SemanticTokenMap? {
    ...
    public func requestFoldingRange(uri: String) async -> [FoldingRange] {
        guard let server else { return [] }
    ...
    public func requestSelectionRange(uri: String, line: Int, character: Int) async -> [SelectionRange] {
        guard let server else { return [] }
        do {
    ...
    public func requestDocumentLinks(uri: String) async -> [DocumentLink] {
        guard let server else { return [] }
    ...
    public func resolveDocumentLinkLSP(_ link: DocumentLink) async -> DocumentLink? {
        guard let server else { return nil }
        do {
    ...
    public func requestDocumentColors(uri: String) async -> [ColorInformation] {
        guard let server else { return [] }
    ...
    public func requestColorPresentation(uri: String, red: Double, green: Double, blue: Double, alpha: Double, range: LSPRange) async -> [ColorPresentation] {
        guard let server else { return [] }
    ...
    @discardableResult
    public func requestCallHierarchyPrepare(uri: String, line: Int, character: Int) async -> [CallHierarchyItem] {
        guard let server else { return [] }
    ...
    public func requestCallHierarchyIncomingCalls(item: CallHierarchyItem) async -> [CallHierarchyIncomingCall] {
        guard let server else { return [] }
        do {
    ...
    public func requestCallHierarchyOutgoingCalls(item: CallHierarchyItem) async -> [CallHierarchyOutgoingCall] {
        guard let server else { return [] }
        do {
    ...
    public var supportsWillSave: Bool {
        guard let server else { return false }
    ...
    public var supportsWillSaveWaitUntil: Bool {
        guard let server else { return false }
        switch server.capabilities.textDocumentSync {
        case .optionA(let options):
    ...
    public nonisolated func parseLocationResponse(_ response: ThreeTypeOption<Location, [Location], [LocationLink]>?) -> Location? {
        guard let response else { return nil }
        switch response {
        case .optionA(let location):
            return location
