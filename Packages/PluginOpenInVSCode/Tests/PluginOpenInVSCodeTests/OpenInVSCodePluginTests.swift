...
@testable import PluginOpenInVSCode
...
func pluginRegistersAndRemovesToolAndDocumentation() throws {
    ...
    let plugin = OpenInVSCodePlugin()

    #expect(plugin.id == "com.coffic.lumi.plugin.open-in-vscode")
    ...
    #expect(tools.allTools().map(\.name) == [OpenInTool.vscode.toolName])
