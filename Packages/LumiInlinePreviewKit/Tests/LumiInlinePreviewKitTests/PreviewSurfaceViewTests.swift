...
final class PreviewSurfaceViewTests: XCTestCase {
    ...
    func test_mouseEvents_whenInteractive_forwardModels() throws {
        ...
        view.isInteractive = true
        var events: [LumiInlinePreviewFacade.PreviewInputEvent] = []
    ...
    func test_layoutAndBackingChanges_notifySize() {
        let view = LumiInlinePreviewFacade.PreviewSurfaceView(frame: NSRect(x: 0, y: 0, width: 123, height: 45))
    ...
    private func makeDraggingPasteboard() -> NSPasteboard {
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("LumiInlinePreviewKitTests-\(UUID().uuidString)"))
