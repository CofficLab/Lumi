...
final class PreviewSurfaceCanvasTests: XCTestCase {
    ...
    func test_init_usesDefaultInteractionAndCallbacks() {
        let canvas = LumiInlinePreviewFacade.PreviewSurfaceCanvas(surfaceID: 123)
    ...
    func test_init_preservesExplicitPropertiesAndCallbacks() {
        ...
        var receivedEvent: LumiInlinePreviewFacade.PreviewInputEvent?

        let canvas = LumiInlinePreviewFacade.PreviewSurfaceCanvas(
            surfaceID: 456,
            isInteractive: true,
    ...
    func test_configure_attachesSurfaceAndAppliesCallbacksAndInteractionState() throws {
        let frame = try XCTUnwrap(
            LumiInlinePreviewFacade.DemoSurfaceFactory.makeFrame(width: 16, height: 12, scale: 1, seq: 1)
        )
        var receivedSize: CGSize?
        var receivedScale: CGFloat?
        var receivedEvent: LumiInlinePreviewFacade.PreviewInputEvent?
        ...
        let view = LumiInlinePreviewFacade.PreviewSurfaceView(frame: NSRect(x: 0, y: 0, width: 16, height: 12))
