...
final class IOSurfaceFrameTests: XCTestCase {
    ...
    func test_frame_roundTripCoding() throws {
        let original = LumiInlinePreviewFacade.IOSurfaceFrame(
            surfaceID: 42,
            width: 320,
            height: 180,
            scale: 2,
        ...
        let decoded = try JSONDecoder().decode(LumiInlinePreviewFacade.IOSurfaceFrame.self, from: data)
        XCTAssertEqual(decoded, original)
    }

    func test_frame_equalityRequiresAllFields() {
        let base = LumiInlinePreviewFacade.IOSurfaceFrame(
