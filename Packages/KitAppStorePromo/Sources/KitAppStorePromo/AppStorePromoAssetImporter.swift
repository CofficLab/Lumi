...
public struct AppStorePromoImportedAsset: Equatable, Sendable {
...
public enum AppStorePromoAssetError: LocalizedError, Equatable {
...
public struct AppStorePromoAssetImporter: Sendable {
    public init() {}
    ...
    public func importImage(
    ...
    ) throws -> AppStorePromoImportedAsset {
        guard fileManager.fileExists(atPath: sourceURL.path) else {
            throw AppStorePromoAssetError.sourceNotFound(sourceURL.path)
        }
        let values = try sourceURL.resourceValues(forKeys: [.fileSizeKey])
        let fileSize = Int64(values.fileSize ?? 0)
        guard fileSize <= 50 * 1024 * 1024 else { throw AppStorePromoAssetError.fileTooLarge(fileSize) }
        guard let source = CGImageSourceCreateWithURL(sourceURL as CFURL, nil),
              let properties = CGImageSourceCopyPropertiesAtIndex(source, 0, nil) as? [CFString: Any],
              let width = properties[kCGImagePropertyPixelWidth] as? Int,
              let height = properties[kCGImagePropertyPixelHeight] as? Int else {
            throw AppStorePromoAssetError.unsupportedImage(sourceURL.path)
        }
        ...
        return AppStorePromoImportedAsset(
            fileURL: destination,
