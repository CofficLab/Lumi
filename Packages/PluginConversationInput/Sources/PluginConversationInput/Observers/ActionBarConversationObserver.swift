...
final class ActionBarConversationObserver: SuperLog {
    ...
    private weak var sender: (any MessageSendingProviding)?
    ...
    init(
        ...
        self.observer = conversations.addSelectedConversationObserver { [weak self] _ in
            ...
            self?.input?.clear()
            // Attachments are a pre-commit global pool. They must not follow
            // the user into another conversation after a switch.
            self?.sender?.clearImageAttachments()
            self?.sender?.clearFileAttachments()
        }
    }
