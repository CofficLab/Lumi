...
public protocol LLMProviderManaging: ObservableObject where ObjectWillChangePublisher == ObservableObjectPublisher {
    ...
    /// 注册单个 LLM Provider
    ///
    /// - Throws: `KernelLumiError.llmProviderRegistrationFailed` 当 provider 的
    ///   `info.id` 为空时。
    ...
    /// - Throws: `KernelLumiError.llmProviderUnavailable` 当内核未注册任何 provider 时
    ...
    /// - Returns: 完整 assistant 消息
    /// - Throws: `KernelLumiError.llmProviderUnavailable` 当内核未注册任何 provider 时
    /// - Throws: `KernelLumiError.invalidProviderOrModel` 当没有选中的 provider 或 model 时
