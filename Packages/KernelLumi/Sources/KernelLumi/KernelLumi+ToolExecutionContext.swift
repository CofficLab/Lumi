...
enum KernelLumiToolExecutionContextStorage {
    @TaskLocal
...
extension KernelLumiContainer {
    public nonisolated func withToolExecutionContextState<T>(
        ...
        try await KernelLumiToolExecutionContextStorage.$current.withValue(state) {
            try await operation()
        }
    }

    private nonisolated var activeToolExecutionState: LumiToolExecutionContextState? {
        KernelLumiToolExecutionContextStorage.current
    }
}
