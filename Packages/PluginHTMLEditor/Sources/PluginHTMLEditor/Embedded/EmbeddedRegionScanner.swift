...
/// HTML 内嵌语言区域定义
public struct HTMLEmbeddedRegion: Equatable, Sendable {
    /// 语言类型
    ...
    /// 虚拟文档内容
    public let virtualContent: String
    /// 行偏移（虚拟文档起始行号）
    ...
    /// 将源文档偏移量转换为虚拟文档偏移量
    public func toVirtualOffset(_ offset: Int) -> Int {
        return offset - startOffset
    }

    /// 将虚拟文档偏移量转换为源文档偏移量
    public func toSourceOffset(_ virtualOffset: Int) -> Int {
        return virtualOffset + startOffset
    }
}

/// 内嵌语言区域扫描器
public enum EmbeddedRegionScanner {
    /// 扫描 HTML 文档中的所有内嵌语言区域
    ///
    /// - Parameter content: HTML 文档内容
    /// - Returns: 内嵌语言区域数组
    public static func scanRegions(in content: String) -> [HTMLEmbeddedRegion] {
    ...
    /// 获取指定偏移量所处的内嵌语言区域
    public static func regionAtOffset(_ offset: Int, in regions: [HTMLEmbeddedRegion]) -> HTMLEmbeddedRegion? {
