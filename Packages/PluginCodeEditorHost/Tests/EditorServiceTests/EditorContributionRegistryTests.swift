...
// 消歧：EditorLanguageRuntime 与 ProviderEditor 存在同名类型；
// EditorContributionBundle 使用中立契约层模型。
import struct ProviderEditor.EditorLanguageDescriptor
import protocol ProviderEditor.LanguageGrammarProviding
...
struct EditorContributionRegistryTests {
    ...
    @Test("右键菜单贡献经 Provider 契约安装，并随插件撤回")
    func contextMenuContributionInstallAndWithdraw() async throws {
        ...
        let provider = TestContextMenuProvider()
        let bundle = EditorContributionBundle(
            pluginID: "context.plugin",
            contextMenuProviders: [provider]
        )
        try await registry.replaceBundle(
            for: "context.plugin",
            with: bundle.stamped(pluginID: "context.plugin", generation: 1)
        )

        let context = EditorKernel.EditorCommandContext(
            languageId: "swift",
            hasSelection: false,
            line: 0,
            character: 0
        )
        let installed = service.editorExtensions.contextMenuSuggestions(
            for: context,
            state: service.state,
            textView: nil
        )
        #expect(installed.contains { $0.id == provider.itemID })

        try await registry.replaceBundle(for: "context.plugin", with: nil)
        let withdrawn = service.editorExtensions.contextMenuSuggestions(
            for: context,
            state: service.state,
            textView: nil
        )
        #expect(!withdrawn.contains { $0.id == provider.itemID })
    }
...
@MainActor
private final class TestContextMenuProvider: EditorContextMenuProviding {
    let id = "provider.context-menu"
    let itemID = "provider.context-menu.item"
