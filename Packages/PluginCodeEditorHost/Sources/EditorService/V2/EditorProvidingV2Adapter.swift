...
/// `EditorProviding` 的 EditorService 实现。
@MainActor
public final class EditorProvidingV2Adapter: EditorProviding, EditorFeatureHostBridge {
    ...
    private lazy var themeCapability = ThemeCapability(adapter: self)
    ...
    public var themes: any EditorThemeProviding { themeCapability }
    ...
    public let extensions: any EditorContributionProviding
    ...
    public init(
        ...
        extensions: (any EditorContributionProviding)? = nil
    ) {
...
@MainActor
private final class ThemeCapability: EditorThemeProviding {
    ...
    var activeThemeID: String? {
        adapter?.service?.theme.currentThemeId
    }

    func install(_ contribution: EditorThemeContribution) {
        guard let registry = adapter?.service?.editorExtensionRegistry else { return }
        registry.registerOrReplaceThemeContributor(
            PaletteSyntaxThemeContributor(
                id: contribution.id,
                displayName: contribution.displayName,
                isDark: contribution.isDark,
                palette: Self.lumiPalette(from: contribution.palette),
    ...
    func uninstall(themeID: String) {
        adapter?.service?.editorExtensionRegistry.unregisterThemeContributor(id: themeID)
    }

    func select(themeID: String) {
        adapter?.service?.theme.syncInitialThemeFromExternal(themeID)
    }

    private static func lumiPalette(from palette: ProviderEditor.EditorSyntaxPalette) -> LumiUI.EditorSyntaxPalette {
        LumiUI.EditorSyntaxPalette(
            text: lumiStyle(palette.text),
            insertionPointHex: palette.insertionPointHex,
            invisibles: lumiStyle(palette.invisibles),
            backgroundHex: palette.backgroundHex,
            lineHighlightHex: palette.lineHighlightHex,
            selectionHex: palette.selectionHex,
            selectionAlpha: palette.selectionAlpha,
            keywords: lumiStyle(palette.keywords),
            commands: lumiStyle(palette.commands),
            types: lumiStyle(palette.types),
            attributes: lumiStyle(palette.attributes),
            variables: lumiStyle(palette.variables),
            values: lumiStyle(palette.values),
            numbers: lumiStyle(palette.numbers),
            strings: lumiStyle(palette.strings),
            characters: lumiStyle(palette.characters),
            comments: lumiStyle(palette.comments)
        )
    }

    private static func lumiStyle(_ style: ProviderEditor.EditorSyntaxTextStyle) -> LumiUI.EditorSyntaxTextStyle {
        LumiUI.EditorSyntaxTextStyle(
...
private final class SessionCapability: EditorSessionProviding {
    ...
    func close(sessionID: EditorSessionID, policy: EditorClosePolicy) async throws {
        ...
        if service.sessionStore.activeSessionID == nil {
