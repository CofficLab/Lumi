...
/// 编辑器贡献包注册表（`EditorContributionProviding` 实现，重构方案 §9）。
///
...
/// 不再使用全局 `reset()` 回放（§9.3）。
@MainActor
public final class EditorContributionRegistry: EditorContributionProviding {
    /// 某插件当前已安装的贡献明细。
    private struct InstalledContribution {
        ...
        let contextMenuProviderIds: [String]
    }
    ...
    // MARK: - EditorContributionProviding

    public func installBundle(for pluginID: String, with bundle: EditorContributionBundle?) throws {
        ...
        for provider in bundle.contextMenuProviders {
            let namespacedID = EditorFeatureBridgeID.namespaced(pluginID: pluginID, providerID: provider.id)
            registry.registerContextMenuContributor(
                KernelContextMenuContributorAdapter(id: namespacedID, base: provider)
            )
            contextMenuIds.append(namespacedID)
        }
        ...
        installed[pluginID] = InstalledContribution(
            ...
            contextMenuProviderIds: contextMenuIds
        )
    ...
    public func replaceBundle(for pluginID: String, with bundle: EditorContributionBundle?) async throws {
        try installBundle(for: pluginID, with: bundle)
    }
    ...
    private func withdraw(pluginID: String) {
        ...
        for providerId in contribution.contextMenuProviderIds {
            registry.unregisterContextMenuContributor(id: providerId)
        }
        for providerId in contribution.completionProviderIds
            + contribution.hoverProviderIds
            + contribution.codeActionProviderIds
            + contribution.quickOpenProviderIds
            + contribution.contextMenuProviderIds {
