...
/// 将 ProviderEditor 的右键菜单贡献桥接到旧编辑器菜单管线。
@MainActor
final class KernelContextMenuContributorAdapter: SuperEditorContextMenuContributor {
    let id: String
    let base: any EditorContextMenuProviding

    init(id: String, base: any EditorContextMenuProviding) {
        self.id = id
        self.base = base
    }
    ...
    func provideContextMenuItems(
        ...
        let selectedText: String? = selection.flatMap { range in
            guard let textView, let stringRange = Range(range, in: textView.string) else { return nil }
            return String(textView.string[stringRange])
        }
        let providerContext = EditorContextMenuContext(
        ...
        return base.provideItems(context: providerContext).map { item in
            let whenClause: EditorWhenClause? = item.requiresSelection ? .key(.hasSelection) : nil
            return EditorContextMenuItemSuggestion(
                id: item.id,
                title: item.title,
