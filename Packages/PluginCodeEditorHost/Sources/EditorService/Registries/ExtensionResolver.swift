...
@MainActor
public final class ExtensionResolver: ObservableObject {

    // MARK: - 单例

    public static let shared = ExtensionResolver()
    ...
    public func resolveCompletion(context: EditorCompletionContext) async -> [EditorCompletionSuggestion] {
    ...
    public func resolveHover(context: EditorHoverContext) async -> [EditorHoverSuggestion] {
    ...
    /// 聚合代码动作（并行请求 + 后台去重）
    public func resolveCodeActions(context: EditorCodeActionContext) async -> [EditorCodeActionSuggestion] {
