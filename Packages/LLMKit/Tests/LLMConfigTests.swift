...
struct LLMConfigTests {
    ...
    @Test("Equatable 相等")
    func equality() {
        let a = LLMConfig(model: "m", providerId: "p")
        let b = LLMConfig(model: "m", providerId: "p")
        #expect(a == b)
    }

    @Test("Equatable 不等 - model 不同")
    func inequalityModel() {
        let a = LLMConfig(model: "m1", providerId: "p")
        let b = LLMConfig(model: "m2", providerId: "p")
        #expect(a != b)
    }

    @Test("Equatable 不等 - temperature 不同")
    func inequalityTemperature() {
        let a = LLMConfig(model: "m", providerId: "p", temperature: 0.5)
        let b = LLMConfig(model: "m", providerId: "p", temperature: 1.0)
        #expect(a != b)
    }
    ...
    func validateTemperatureTooHigh() {
        let config = LLMConfig(model: "m", providerId: "p", temperature: 2.5)
        #expect {
            try config.validate()
        } throws: { error in
            guard let e = error as? LLMServiceError,
    ...
    func validateTemperatureBoundaries() throws {
        let lo = LLMConfig(model: "m", providerId: "p", temperature: 0.0)
        let hi = LLMConfig(model: "m", providerId: "p", temperature: 2.0)
    ...
    func validateMaxTokensInvalid() {
        let config = LLMConfig(model: "m", providerId: "p", maxTokens: 0)
        #expect {
            try config.validate()
        } throws: { error in
            guard let e = error as? LLMServiceError,
    ...
    func validateMaxTokensValid() throws {
        let config = LLMConfig(model: "m", providerId: "p", maxTokens: 4096)
        #expect(throws: Never.self) { try config.validate() }
    }

    @Test("validate 通过 - nil temperature 和 nil maxTokens")
    func validateNilOptionals() throws {
        let config = LLMConfig(model: "m", providerId: "p")
        #expect(throws: Never.self) { try config.validate() }
    }
}
