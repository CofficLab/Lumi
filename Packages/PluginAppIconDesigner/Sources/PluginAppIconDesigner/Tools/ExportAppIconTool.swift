...
public struct ExportAppIconTool: SuperAgentTool {
    ...
    public func description(for language: LanguagePreference) -> String {
        "Export the current App Icon Designer document or candidate image as an Xcode 26 AppIcon.icon that supports macOS 15 and later."
    }
    ...
    public func inputSchema(for language: LanguagePreference) -> [String: Any] {
        ...
        return ["type": "object", "properties": properties, "required": ["outputDirectory"]]
    }
    ...
    public func execute(arguments: [String: ToolArgument]) async throws -> String {
        ...
        guard let outputDirectory = IconToolSupport.string(arguments, "outputDirectory"), !outputDirectory.isEmpty else {
            return IconToolSupport.missingParameter("outputDirectory", language: language)
        }

        let requestedArtifactId = IconToolSupport.string(arguments, "artifactId")
        let explicitDocumentId = IconToolSupport.string(arguments, "documentId")?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let setName = IconToolSupport.string(arguments, "setName")?
            .trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func exportSuccessMessage(language: LanguagePreference, url: URL) -> String {
