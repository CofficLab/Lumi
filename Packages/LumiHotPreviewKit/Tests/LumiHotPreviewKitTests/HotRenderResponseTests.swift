...
struct HotRenderResponseTests {
    ...
    func wrapsLegacyRenderResponse() {
        ...
        let response = LumiHotPreviewPackage.HotRenderResponse(legacy)
    ...
    func preferredTransportPriority() {
        #expect(LumiHotPreviewPackage.HotRenderResponse(success: true).preferredTransport == .none)
        #expect(LumiHotPreviewPackage.HotRenderResponse(success: true, previewImagePNGBase64: "png").preferredTransport == .base64)
        #expect(LumiHotPreviewPackage.HotRenderResponse(success: true, previewImagePNGBase64: "png", imageFilePath: "/tmp/frame.png").preferredTransport == .file)
        #expect(LumiHotPreviewPackage.HotRenderResponse(success: true, previewImagePNGBase64: "png", imageFilePath: "/tmp/frame.png", sharedMemoryTag: "tag").preferredTransport == .sharedMemory)
    }
    ...
    func decodesNewHostFields() throws {
        let data = Data("""
        {
          "success": true,
          "previewID": "preview-2",
          "imageFilePath": "/tmp/LumiHotPreviewKit/frame.png",
        ...
        let response = try JSONDecoder().decode(LumiHotPreviewPackage.HotRenderResponse.self, from: data)

        #expect(response.success)
        #expect(response.previewID == "preview-2")
        #expect(response.imageFilePath == "/tmp/LumiHotPreviewKit/frame.png")
