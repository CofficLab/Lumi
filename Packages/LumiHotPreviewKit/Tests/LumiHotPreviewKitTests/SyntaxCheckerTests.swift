...
struct SyntaxCheckerTests {
    ...
    func returnsValidWhenCommandSucceeds() async {
        ...
        let checker = LumiHotPreviewPackage.SyntaxChecker(swiftcPath: "/fake/swiftc", runner: runner)
    ...
    func returnsDiagnosticsWhenCommandFails() async throws {
        ...
        let checker = LumiHotPreviewPackage.SyntaxChecker(runner: runner)
...
private actor FakeCommandRunner: LumiHotPreviewPackage.CommandRunning {
    private let result: LumiHotPreviewPackage.CommandResult?
    private let error: Error?
    private(set) var commands: [[String]] = []

    init(result: LumiHotPreviewPackage.CommandResult) {
        self.result = result
        self.error = nil
    }
    ...
    func run(_ command: [String]) async throws -> LumiHotPreviewPackage.CommandResult {
