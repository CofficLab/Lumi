...
public extension LumiHotPreviewPackage {
    ...
    final class IncrementalBuildPipeline: Sendable {
        private let incrementalCompiler: LumiPreviewPackage.IncrementalCompiler
        private let xcodeCompiler: LumiPreviewPackage.XcodeCompiler
        private let compilerArgumentResolver: @Sendable (LumiPreviewPackage.BuildStrategy) async throws -> [String]
        private let moduleNameResolver: @Sendable (LumiPreviewPackage.BuildStrategy) async throws -> String?
        private let moduleImportPlanCache: ModuleImportPlanCache

        public init(
            incrementalCompiler: LumiPreviewPackage.IncrementalCompiler = .init(),
            xcodeCompiler: LumiPreviewPackage.XcodeCompiler = .init(),
            compilerArgumentResolver: (@Sendable (LumiPreviewPackage.BuildStrategy) async throws -> [String])? = nil,
            moduleNameResolver: (@Sendable (LumiPreviewPackage.BuildStrategy) async throws -> String?)? = nil,
            ...
            self.compilerArgumentResolver = compilerArgumentResolver
                ?? Self.defaultCompilerArgumentResolver(
                    spmCompiler: LumiPreviewPackage.SPMCompiler(),
        ...
        public func linkPreviewEntry(objectURLs: [URL]) async throws -> URL {
            ...
            guard process.terminationStatus == 0,
                ...
                throw LumiPreviewPackage.PreviewError.compilationFailed(
        ...
        public func compilePreviewEntryImportingModule(
            ...
            let directory = FileManager.default.temporaryDirectory
                .appendingPathComponent("LumiHotPreviewKit-ImportEntry-\(UUID().uuidString)", isDirectory: true)
        ...
        private func inferModuleName(
            from buildStrategy: LumiPreviewPackage.BuildStrategy,
            compilerArguments: [String]
        ) async throws -> String {
        ...
        private static func defaultCompilerArgumentResolver(
            spmCompiler: LumiPreviewPackage.SPMCompiler,
            xcodeCompiler: LumiPreviewPackage.XcodeCompiler
        ) -> @Sendable (LumiPreviewPackage.BuildStrategy) async throws -> [String] {
            { buildStrategy in
                switch buildStrategy {
        ...
        private static func defaultModuleNameResolver() -> @Sendable (LumiPreviewPackage.BuildStrategy) async throws -> String? {
            { buildStrategy in
                switch buildStrategy {
                case .spm(_, let targetName):
                    return targetName
