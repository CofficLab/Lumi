...
extension SuperPlugin {
    ...
    /// 默认实现：不提供工具栏前导视图
    @MainActor public func addToolBarLeadingView(context: PluginContext) -> AnyView? { nil }

    /// 默认实现：不提供工具栏中间视图
    @MainActor public func addToolBarCenterView(context: PluginContext) -> AnyView? { nil }

    /// 默认实现：不提供工具栏右侧视图
    @MainActor public func addToolBarTrailingView(context: PluginContext) -> AnyView? { nil }

    /// 默认实现：不提供 Activity Bar 视图容器
    @MainActor public func addViewContainer() -> ViewContainerItem? { nil }

    /// 默认实现：不提供 Panel Header 视图
    @MainActor public func addPanelHeaderView(context: PluginContext) -> AnyView? { nil }

    /// 默认实现：不提供底部面板标签页
    @MainActor public func addBottomPanelTabs(context: PluginContext) -> [BottomPanelTab] { [] }

    /// 默认实现：不提供底部面板内容视图
    @MainActor public func addBottomPanelContentView(tabId: String, context: PluginContext) -> AnyView? { nil }

    /// 默认实现：不提供 Rail 标签页
    @MainActor public func addRailTabs(context: PluginContext) -> [RailTab] { [] }

    /// 默认实现：不提供 Rail 内容视图
    @MainActor public func addRailContentView(tabId: String, context: PluginContext) -> AnyView? { nil }
    ...
    /// 默认实现：不提供右侧栏底部工具栏右侧项
    @MainActor public func addSidebarTrailingToolbarItems(context: PluginContext) -> [SidebarToolbarItem] { [] }

    /// 默认实现：不提供右侧栏工具栏项的自定义按钮视图
    @MainActor public func addSidebarToolbarItemView(itemId: String, context: PluginContext) -> AnyView? { nil }
