...
public enum ErrorDispositionResolver {
    ...
    private static func extractStatusCode(from error: Error) -> Int? {
        ...
        if case let LumiLLMProviderSupportError.streamingFailed(message) = error {
            return LLMHTTPErrorParsing.statusCode(from: message)
        }
        return nil
    }
}

public enum LLMHTTPErrorParsing {
...
public enum LLMProviderErrorSupport {
    public static func makeErrorMessage(
        ...
        if case let LumiLLMProviderSupportError.streamingFailed(message) = error {
            ...
            detail = LumiLLMFailureDetail(
                ...
                httpStatusCode: LLMHTTPErrorParsing.statusCode(from: split.summary)
                    ?? LLMHTTPErrorParsing.statusCode(from: message),
                transportDetails: nil
            )
            metadata = LLMTransportDetails.metadata(from: split)
        } else {
            detail = LLMFailureDetailResolver.resolve(from: error)
            if let transportDetails = detail.transportDetails {
                metadata[LLMTransportMetadata.responseDetails] = transportDetails
            }
        }
