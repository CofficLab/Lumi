...
public enum StreamingRequestSupport {
    ...
    private static func attemptAnthropicStreamingRequest(
        ...
        do {
            try await apiService.sendStreamingRequest(
                ...
                onChunk: { chunkData in
                    let shouldContinue = await processAnthropicChunk(
                        chunkData: chunkData,
                        parse: { try adapter.parseStreamChunk(data: $0) },
                        state: state,
                        onChunk: onChunk
    ...
    private static func processOpenAIChunk(
        chunkData: Data,
    ...
    private static func processAnthropicChunk(
        chunkData: Data,
        parse: (Data) throws -> StreamChunk?,
        state: StreamingState,
        onChunk: @Sendable (LumiStreamChunk) async -> Void
    ...
    private static func attachTransportDetails(
        ...
        let details = await TransportDetailsSupport.transportDetails(
            request: request,
