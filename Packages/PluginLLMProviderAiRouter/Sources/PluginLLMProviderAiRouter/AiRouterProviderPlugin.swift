...
/// AiRouter 供应商装配插件（KernelCore 生态）。
///
/// 在 `onBoot` 中把本供应商的 AiRouterProvider 注册进
/// `LLMProviderManagerProviding`，聊天链路即可经管理器路由到该供应商。
@MainActor
public final class AiRouterProviderPlugin: SuperPlugin, SuperLog {
    nonisolated static let logger = Logger(subsystem: "com.coffic.lumi.plugin.llm-provider.airouter", category: "AiRouter")
    ...
    public func onBoot(kernel: KernelCoreContainer) throws {
        ...
        let providers: [any SuperLLMProvider] = [AiRouterProvider(networkProvider: networkProvider)]
