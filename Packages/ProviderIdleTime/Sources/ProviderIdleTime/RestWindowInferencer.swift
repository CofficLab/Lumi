...
/// 推断画像：工作日 / 周末 / 全局回退。
public enum IdleInferenceProfile: Sendable {
    case weekday
    case weekend
    case global

    public var source: RestWindowSource {
        switch self {
    ...
    public var targetObservedDays: Int {
        switch self {
...
/// 从历史活动事件推断「休息窗口」（一天中的低活跃时段）。
///
/// 由旧版 `Plugins/IdleTimePlugin/Sources/Services/RestWindowInferencer.swift` 迁移而来。
public struct RestWindowInferencer: Sendable {
    public static let bucketsPerDay = 48
    public static let bucketMinutes = 30
    ...
    public func infer(events: [IdleActivityEvent], now: Date = Date()) -> IdleInferenceSnapshot {
