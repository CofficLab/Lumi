...
/// 开发者活动的磁盘存储：事件追加 / 最近事件读取 / 快照读写 / 过期清理。
///
/// 由旧版 `Plugins/IdleTimePlugin/Sources/Services/IdleActivityStore.swift` 迁移而来。
/// 差异：不再依赖 `IdleTimeRuntimeBridge` 的全局目录桥，目录由宿主直接注入
/// （插件 onBoot 时用 StorageProviding 的插件数据目录构造，否则回退临时目录）。
    ...
    public init(
        ...
        let root = directoryURL
            ?? FileManager.default.temporaryDirectory
                .appendingPathComponent("ProviderIdleTime", isDirectory: true)
