...
public struct LumiModelCapabilities: Sendable, Equatable, Hashable {
    ...
    /// 模型对思考与推理强度的支持级别（大统一字段，全仓库唯一建模入口）。
    public let thinkingAndReasoning: LumiThinkingAndReasoning

    public init(
        supportsVision: Bool,
        supportsTools: Bool,
        supportsTTS: Bool = false,
        thinkingAndReasoning: LumiThinkingAndReasoning = .unsupported
    ) {
        self.supportsVision = supportsVision
        self.supportsTools = supportsTools
        self.supportsTTS = supportsTTS
        self.thinkingAndReasoning = thinkingAndReasoning
    }
}
...
extension LumiReasoningEffort {
    ...
    ///   均返回空数组。
    public static func available(for support: LumiThinkingAndReasoning) -> [LumiReasoningEffort] {
        switch support {
        case .unsupported: []
        case .toggle: []
