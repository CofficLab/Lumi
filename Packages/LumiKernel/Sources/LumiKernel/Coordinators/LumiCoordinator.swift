...
/// 内核协调器注册表
///
/// `LumiKernel.startup()` 在插件服务注册完毕(onBoot)之后、服务校验之前,
...
public final class LumiCoordinatorRegistry {
    ...
    /// 校验依赖并按注册顺序启动全部协调器。
    ///
    /// 任一协调器依赖缺失时抛出 `LumiKernelError.missingRequiredServices`,
    /// 带上协调器 id,便于定位是哪条链路没装配起来。
    public func startAll(kernel: LumiKernel) throws {
        for coordinator in coordinators {
            let missing = coordinator.requiredServices
                .filter { !kernel.isServiceRegistered($0.id) }
                .map(\.name)
            guard missing.isEmpty else {
                throw LumiKernelError.missingRequiredServices(
