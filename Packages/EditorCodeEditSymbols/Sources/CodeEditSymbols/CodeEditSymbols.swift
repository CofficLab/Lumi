...
// In some SwiftPM CLI flows, `Bundle.module` for resource-bearing targets may not be
// generated as expected. We fall back to a bundle resolved from this module type.
private final class CodeEditSymbolsBundleToken {}

private enum CodeEditSymbolsBundle {
    static var current: Bundle {
        let hostBundle = Bundle(for: CodeEditSymbolsBundleToken.self)
        // Prefer the SwiftPM module bundle when available, but make this resilient
        // to bundle naming differences across SwiftPM/Xcode flows.
        let resourceBaseURL = hostBundle.resourceURL
        let candidates = [
            // Likely shape when package name changed.
            "EditorCodeEditSymbols_CodeEditSymbols.bundle",
            // Fallback for the original upstream package name.
            "CodeEditSymbols_CodeEditSymbols.bundle",
        ]

        if let resourceBaseURL {
            for name in candidates {
                let moduleURL = resourceBaseURL.appendingPathComponent(name)
...
public extension Image {
    ...
    init(symbol: String) {
        self.init(symbol, bundle: CodeEditSymbolsBundle.current)
    }
...
public extension NSImage {
    ...
    static func symbol(named: String) -> NSImage? {
        CodeEditSymbolsBundle.current.image(forResource: named)
    }
