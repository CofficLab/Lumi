...
final class HotStdioPreviewHost {
    ...
    private var lastCursorShape: LumiPreviewFacade.PreviewCursorShape = .arrow
    ...
    private func handleLine(_ data: Data) {
        do {
            let request = try decoder.decode(LumiPreviewFacade.HostRequest.self, from: data)
    ...
    private func dispatch(_ command: LumiPreviewFacade.HostCommand) -> LumiPreviewFacade.HostResponse {
        switch command {
        case .ping:
    ...
    private func sendResponse(requestID: UInt64, payload: LumiPreviewFacade.HostResponse) {
        write(.response(requestID: requestID, payload: payload))
    }

    private func sendEvent(_ event: LumiPreviewFacade.HostEvent) {
        write(.event(event))
    }

    private func publishCurrentCursorIfNeeded() {
        sendCursorChanged(LumiPreviewFacade.PreviewCursorShape(appKit: NSCursor.current))
    }

    private func sendCursorChanged(_ shape: LumiPreviewFacade.PreviewCursorShape) {
        guard lastCursorShape != shape else { return }
        lastCursorShape = shape
        sendEvent(.cursorChanged(shape))
    }

    private func write(_ outbound: LumiPreviewFacade.HostOutbound) {
