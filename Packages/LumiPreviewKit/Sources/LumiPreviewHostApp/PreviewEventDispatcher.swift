...
final class PreviewEventDispatcher {
    ...
    private func dispatchMouse(
        _ event: LumiPreviewFacade.MouseEvent,
    ...
    private func dispatchScroll(
        _ event: LumiPreviewFacade.ScrollWheelEvent,
        into window: NSWindow
    ) {
    ...
    private func dispatchKey(
        _ event: LumiPreviewFacade.KeyEvent,
        into window: NSWindow
    ) {
    ...
    private func dispatchFlagsChanged(
        modifiers: LumiPreviewFacade.ModifierFlags,
        into window: NSWindow
    ) {
    ...
    private func dispatchTextInput(
        _ event: LumiPreviewFacade.TextInputEvent,
        into window: NSWindow
    ) {
    ...
    private func dispatchDragAndDrop(
        _ event: LumiPreviewFacade.DragDropEvent,
        into window: NSWindow
    ) {
        let location = NSPoint(x: event.x, y: event.y)
    ...
    private func dispatchTouchBar(
        _ event: LumiPreviewFacade.TouchBarEvent,
        into window: NSWindow
    ) {
    ...
    private static func makePasteboard(for items: [LumiPreviewFacade.DragDropEvent.Item]) -> NSPasteboard {
...
// 见 `Models/PreviewInputEvent+AppKit.swift`，子进程通过 `import LumiPreviewKit` 引入。
