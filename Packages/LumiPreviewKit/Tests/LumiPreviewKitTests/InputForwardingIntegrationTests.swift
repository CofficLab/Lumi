...
private actor InputProbe {
    ...
    func observePolicy(_ policy: LumiPreviewFacade.FrameStreamPolicy) -> (idle: Bool, inputInteractive: Bool) {
        if policy == .idle {
            return (true, false)
        }
...
final class InputForwardingIntegrationTests: XCTestCase {
    ...
    func test_forwardInputEvent_acceptsAllShapes_andKeepsSubprocessAlive() async throws {
        ...
        let eventsToSend: [LumiPreviewFacade.PreviewInputEvent] = [
    ...
    func test_forwardInputEvent_promotesIdleStreamBackToInteractive() async throws {
        ...
        let inputEvent = LumiPreviewFacade.PreviewInputEvent.mouse(.init(
            phase: .moved,
            button: .left,
            x: 80,
            y: 80,
