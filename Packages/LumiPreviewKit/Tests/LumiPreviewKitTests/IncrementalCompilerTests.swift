...
struct IncrementalCompilerTests {
    ...
    func compileSingleFileReturnsObjectFile() async throws {
        ...
        let compiledObjectURL = try await LumiPreviewPackage.IncrementalCompiler().compile(
    ...
    func compileSyntaxErrorThrowsCompilationFailed() async throws {
        ...
        do {
            _ = try await LumiPreviewPackage.IncrementalCompiler().compile(
                fileURL: fixture.file,
                compileCommand: command
            )
    ...
    func compileLibraryUsesCompilerArguments() async throws {
        ...
        let compiledDylibURL = try await LumiPreviewPackage.IncrementalCompiler().compileLibrary(
    ...
    private func makeTemporarySwiftFile(source: String) throws -> (directory: URL, file: URL) {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("LumiPreviewKit-LumiPreviewPackage.IncrementalCompiler-\(UUID().uuidString)", isDirectory: true)
