...
@Suite("LivePreviewFrameAlignment")
struct LivePreviewFrameAlignmentTests {

    @Test("scale 2 时按半像素边界对齐")
    func alignsToHalfPointsAtScale2() {
        let frame = NSRect(x: 10.24, y: 20.26, width: 300.24, height: 200.24)
        let aligned = LumiPreviewPackage.LivePreviewFrameAlignment.pixelAlignedFrame(frame, scale: 2)
    ...
    func clampsScaleBelowOne() {
        ...
        let aligned = LumiPreviewPackage.LivePreviewFrameAlignment.pixelAlignedFrame(frame, scale: 0.5)
