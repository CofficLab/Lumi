...
private actor FrameCollector {
    private(set) var frames: [LumiPreviewFacade.IOSurfaceFrame] = []
    ...
    private(set) var cursorShapes: [LumiPreviewFacade.PreviewCursorShape] = []

    func append(_ frame: LumiPreviewFacade.IOSurfaceFrame) {
        frames.append(frame)
    }
    func notePolicy(_ policy: LumiPreviewFacade.FrameStreamPolicy) {
    ...
    func noteCursorShape(_ shape: LumiPreviewFacade.PreviewCursorShape) {
        cursorShapes.append(shape)
    }
    var firstFrame: LumiPreviewFacade.IOSurfaceFrame? { frames.first }
...
final class HostConnectionIntegrationTests: XCTestCase {
    ...
    /// 用 `swiftc` 把 fixture 源文件编译成 `/tmp/LumiPreviewFixture-<uuid>.dylib`。
    private func compileFixtureDylib() throws -> URL {
        ...
        let outputURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("LumiPreviewFixture-\(UUID().uuidString).dylib")
