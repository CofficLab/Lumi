...
final class InlinePreviewSessionTests: XCTestCase {
    ...
    func test_init_startsNotRunning() {
        let session = LumiInlinePreviewFacade.InlinePreviewSession()
    ...
    func test_startAndFrameStream_deliversCallbacks() async throws {
        guard LumiInlinePreviewFacade.InlineHostExecutableResolver.resolve() != nil else {
        ...
        var receivedFrame: LumiInlinePreviewFacade.IOSurfaceFrame?
        var receivedPolicy: LumiInlinePreviewFacade.FrameStreamPolicy?
    ...
    func test_sessionErrorDescriptions() {
        XCTAssertEqual(
            LumiInlinePreviewFacade.InlinePreviewSession.SessionError.executableNotFound.errorDescription,
            "LumiInlinePreviewHostApp executable not found. Set the LUMI_INLINE_PREVIEW_HOST_PATH environment variable or embed it in the app bundle."
        )
        ...
        XCTAssertEqual(
            LumiInlinePreviewFacade.InlinePreviewSession.SessionError.underlying(underlying).errorDescription,
            "fixture failure"
        )
    }

    private func assertExecutableNotFound(
        _ operation: () async throws -> LumiInlinePreviewFacade.HostResponse,
        ...
        } catch LumiInlinePreviewFacade.InlinePreviewSession.SessionError.executableNotFound {
            // Expected.
        } catch {
