...
public final class TextActionsSuperPlugin: SuperPlugin, SuperLog {
    ...
    public func onBoot(kernel: KernelCoreContainer) throws {
        ...
        if let docs = kernel.resolveProvider((any DocsViewProviding).self) { docs.addAbout(DocsEntry(id: id, name: metadata.name) { TextActionsAboutView() }); docs.addManual(DocsEntry(id: id, name: metadata.name) { TextActionsManualView() }) }
        if TextActionsSettings.isEnabled { TextSelectionManager.shared.startMonitoring() }
    }
    public func onEnable(kernel: KernelCoreContainer) async throws { if TextActionsSettings.isEnabled { TextSelectionManager.shared.startMonitoring() } }
    public func onDisable(kernel: KernelCoreContainer) async throws { TextSelectionManager.shared.stopMonitoring() }
    public func onShutdown(kernel: KernelCoreContainer) throws { TextSelectionManager.shared.stopMonitoring(); TextActionMenuController.shared.hide(); kernel.resolveProvider((any SettingViewProviding).self)?.removeEntries(ids: [id]); kernel.resolveProvider((any DocsViewProviding).self)?.removeEntries(id: id) }
}
