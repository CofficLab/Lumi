...
struct PromoImageRowView: View {
    ...
    init(workspace: WorkspaceStore, task: AppStorePromoTask, image: AppStorePromoImage) {
    ...
    var body: some View {
        ...
        AppListRow(isSelected: isSelected, action: {
            workspace.select(taskID: task.id, imageID: image.id)
        }) {
            HStack(spacing: 8) {
                Text("\(image.order + 1)")
                    .font(.appMonoMicro)
        ...
        .contextMenu {
            Button(role: .destructive) {
                workspace.deleteImage(taskID: task.id, imageID: image.id)
            } label: {
