...
public struct ExportPromoTaskTool: SuperAgentTool {
    public let name = "app_store_promo_export_task"

    public init() {}

    public func description(for language: LanguagePreference) -> String {
        "Render every HTML image in a task to an explicitly selected external directory."
    }
    ...
    public func inputSchema(for language: LanguagePreference) -> [String: Any] {
        ...
        return ["type": "object", "properties": properties, "required": ["taskId", "outputDirectory"]]
    }

    public func displayDescription(for arguments: [String: ToolArgument]) -> String {
        PromoToolSupport.localized(PromoToolSupport.language, en: "Export promo task", zh: "导出促销图任务")
    }

    public func permissionRiskLevel(arguments: [String: ToolArgument]) -> CommandRiskLevel {
        PromoToolSupport.bool(arguments, "overwrite") ? .high : .medium
    }
    ...
    public func execute(arguments: [String: ToolArgument]) async throws -> String {
        ...
        let requestedTypes = PromoToolSupport.stringArray(arguments, "displayTypes")
            ?? AppStorePromoDisplaySpec.presets(for: task.deviceFamily).map(\.displayType)
        ...
        let overwrite = PromoToolSupport.bool(arguments, "overwrite")
