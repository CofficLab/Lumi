...
/// OpenAI 供应商装配插件（KernelCore 生态）。
///
/// 在 `onBoot` 中把本供应商的 OpenAIProvider 注册进
/// `LLMProviderManagerProviding`，聊天链路即可经管理器路由到该供应商。
@MainActor
public final class OpenAIProviderPlugin: SuperPlugin, SuperLog {
    nonisolated static let logger = Logger(subsystem: "com.coffic.lumi.plugin.llm-provider.openai", category: "OpenAI")
    ...
    public let metadata = PluginMetadata(
        ...
        name: "OpenAI 供应商",
        description: "注册 OpenAIProvider 到 LLM 管理器。",
        category: .llm,
        stage: .stable,
        policy: .alwaysOn
    )
    ...
    public func onBoot(kernel: KernelCoreContainer) throws {
        ...
        let providers: [any SuperLLMProvider] = [OpenAIProvider(apiService: apiService)]
