...
@Suite
@MainActor
struct EditorTextViewTextViewTests {
    ...
    func delegateChangesText() {
        ...
        // The line storage should still be consistent after a re-entrant edit
        #expect(textView.layoutManager.lineCount == 1)
        #expect(textView.layoutManager.lineStorage.length == 23)
    }
    ...
    @Test("Asymmetric replacement keeps later cursor position correct")
    func asymmetricReplacementUpdatesLaterCursors() {
        let textView = TextView(string: "abcdef")
        textView.selectionManager.setSelectedRange(NSRange(location: 5, length: 0))

        // Replace 2 characters with 3: cursor after the edit should shift by +1
        textView.replaceCharacters(in: NSRange(location: 1, length: 2), with: "XYZ")

        #expect(textView.string == "aXYZdef")
        #expect(
            textView.selectionManager.textSelections.map(\.range) == [NSRange(location: 6, length: 0)]
        )
    }
}
