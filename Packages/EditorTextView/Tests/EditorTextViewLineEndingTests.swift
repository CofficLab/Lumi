...
final class EditorTextViewLineEndingTests: XCTestCase {
    func test_lineEndingCreateUnix() {
        ...
        XCTAssertEqual(LineEnding(line: line), .lineFeed)
        XCTAssertEqual(LineEnding.lineFeed.length, 1)
    }
    ...
    func test_lineEndingCreateCRLF() {
        ...
        let line = "Loren Ipsum\r\n"
        XCTAssertTrue(LineEnding(line: line) != nil, "Line ending failed to initialize with a line ending in \\r\\n")
        XCTAssertEqual(LineEnding(line: line), .carriageReturnLineFeed)
        XCTAssertEqual(LineEnding.carriageReturnLineFeed.length, 2)
    }
    ...
    func test_lineEndingCreateMacOS() {
        ...
        XCTAssertEqual(LineEnding(line: line), .carriageReturn)
        XCTAssertEqual(LineEnding.carriageReturn.length, 1)
    }

    func test_lineEndingRejectsTextWithoutLineEnding() {
        XCTAssertNil(LineEnding(line: "Loren Ipsum"))
        XCTAssertNil(LineEnding(line: ""))
    }
    ...
    func test_detectLineEndingCRLF() {
    ...
    func test_detectLineEndingTieBreaksToLineFeed() {
        // Equal counts of two line endings should default to .lineFeed
        let storage = NSTextStorage(string: "a\nb\rc\nd\r")
        let lineStorage = TextLineStorage<TextLine>()
        lineStorage.buildFromTextStorage(storage, estimatedLineHeight: 10)

        XCTAssertEqual(LineEnding.detectLineEnding(lineStorage: lineStorage, textStorage: storage), .lineFeed)
    }
}
