...
/// 按 id 加载思维导图到画布并选中。
public struct LoadMindMapTool: SuperAgentTool {
    public let name = "load_mind_map"

    public init() {}

    public func description(for language: LanguagePreference) -> String {
        "Load (select) a mind map by id so it shows on the canvas. Use list_mind_maps to discover ids."
    }
    ...
    public func inputSchema(for language: LanguagePreference) -> [String: Any] {
        ...
        return ["type": "object", "properties": properties, "required": ["mapId"]]
    }

    public func displayDescription(for arguments: [String: ToolArgument]) -> String {
        MindMapToolSupport.localized(MindMapToolSupport.language, en: "Load mind map", zh: "加载思维导图")
    }
    ...
    public func execute(arguments: [String: ToolArgument]) async throws -> String {
        ...
        guard let mapId = MindMapToolSupport.nonEmpty(MindMapToolSupport.string(arguments, "mapId")) else {
