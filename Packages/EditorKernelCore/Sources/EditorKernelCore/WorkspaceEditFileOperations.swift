...
public actor WorkspaceEditFileOperationsExecutor {

    // MARK: - 单例

    public static let shared = WorkspaceEditFileOperationsExecutor()
    ...
    public func applyCreateFile(_ operation: CreateFile) async -> Bool {
    ...
    public func applyRenameFile(_ operation: RenameFile) async -> Bool {
    ...
    public func applyDeleteFile(_ operation: DeleteFile) async -> Bool {
    ...
    /// 应用 CreateFile（直接调用）
    public func applyCreateFile(uri: String, overwrite: Bool, ignoreIfExists: Bool) async -> Bool {
    ...
    /// 应用 DeleteFile（直接调用）
    public func applyDeleteFile(uri: String, recursive: Bool, ignoreIfNotExists: Bool) async -> Bool {
    ...
    /// 互不依赖的操作可以并行执行
    public func applyWorkspaceEdit(_ edit: WorkspaceEdit) async throws {
        var errors: [Error] = []
    ...
    public enum WorkspaceEditError: Error, LocalizedError {
        case createFailed
        case deleteFailed
        case renameFailed
...
public enum WorkspaceEditFileOperations {
    public static func applyCreateFile(_ operation: CreateFile) -> Bool {
        applyCreateFile(
            uri: operation.uri,
            overwrite: operation.options?.overwrite == true,
            ignoreIfExists: operation.options?.ignoreIfExists == true
    ...
    public static func applyRenameFile(_ operation: RenameFile) -> Bool {
        applyRenameFile(
            oldURI: operation.oldUri,
            newURI: operation.newUri,
            overwrite: operation.options.overwrite == true,
    ...
    public static func applyDeleteFile(_ operation: DeleteFile) -> Bool {
        applyDeleteFile(
            uri: operation.uri,
            recursive: operation.options.recursive == true,
            ignoreIfNotExists: operation.options.ignoreIfNotExists == true
    ...
    public static func applyCreateFile(uri: String, overwrite: Bool, ignoreIfExists: Bool) -> Bool {
    ...
    public static func applyRenameFile(
        oldURI: String,
        newURI: String,
        overwrite: Bool,
        ignoreIfExists: Bool
    ...
    public static func applyDeleteFile(uri: String, recursive: Bool, ignoreIfNotExists: Bool) -> Bool {
