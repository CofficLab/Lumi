...
/// 本地 Web 服务能力协议
///
/// 提供一个仅监听本地回环地址(默认 127.0.0.1)的 HTTP 服务,聚合所有插件贡献的
/// `WebRoute`。插件的能力(如切换主题)由此暴露为可被本地工具调用的 HTTP API。
///
/// 协议刻意 **不** 标 `@MainActor`:套接字监听与连接处理在后台线程进行,不应占用
/// 主线程。路由注册(`register`/`unregister`)由宿主在主线程**同步**发起(与
/// 其它贡献收集逻辑一致);具体实现需自行保证路由表的线程安全(如用锁),因为请求
/// 匹配会在后台线程并发读取该表。
///
/// 与出站 HTTP 客户端能力互补:本协议是入站服务端。
