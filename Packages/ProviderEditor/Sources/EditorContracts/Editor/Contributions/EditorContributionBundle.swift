...
public struct EditorContributionBundle {
    ...
    /// 右键菜单贡献。
    public let contextMenuProviders: [any EditorContextMenuProviding]
    ...
    public init(
        ...
        contextMenuProviders: [any EditorContextMenuProviding] = []
    ) {
        ...
        self.contextMenuProviders = contextMenuProviders
    }
    ...
    public var validationIssues: [String] {
        ...
        let contextMenuIDs = contextMenuProviders.map { $0.id }
        if Set(contextMenuIDs).count != contextMenuIDs.count {
            issues.append("duplicate context menu provider ids: \(contextMenuIDs)")
        }
        return issues
    }
}
