...
/// 编辑器 Host Capability 总入口。
///
/// 见 `docs/editor-kernel-plugin-rearchitecture-plan.md` §8。
/// 本协议由 `EditorHostPlugin` 实现并注册。所有编辑器能力通过本入口
/// 暴露为独立的 capability，具体实现仍由 Host 持有。
///
/// 并发与观察规则（§8.8）：
...
public protocol EditorProviding: AnyObject {
    ...
    /// 主题能力：主题贡献注册、撤回与选择。
    var themes: any EditorThemeProviding { get }
    ...
    var extensions: any EditorContributionProviding { get }
}

/// 迁移期间保留的兼容名称。
@available(*, deprecated, renamed: "EditorProviding")
public typealias EditorProvidingV2 = EditorProviding
