...
///   `MessageSendingProviding` 加入文件附件挂起池。聊天输入框中的图片和文件拖放由
///   `PluginConversationInput` 负责加入对应的附件挂起池。
///
/// 与旧版的对应关系：
/// - `chatSectionActionBarItems(.leading)` → `ChatSectionProviding.addBarItems(.actionLeading)`；
/// - 文件选择器直接复用当前 `MessageSendingProviding.addFileAttachment` 接口；
/// - 图片和文件拖放的附件管线保留在 `PluginConversationInput`，复用当前发送器接口。
@MainActor
