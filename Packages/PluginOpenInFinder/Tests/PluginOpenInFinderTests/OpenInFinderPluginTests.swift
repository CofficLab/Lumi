...
@testable import PluginOpenInFinder
...
func pluginRegistersAndRemovesToolAndDocumentation() throws {
    ...
    let plugin = OpenInFinderPlugin()

    #expect(plugin.id == "com.coffic.lumi.plugin.open-in-finder")
    ...
    #expect(tools.allTools().map(\.name) == [OpenInTool.finder.toolName])
