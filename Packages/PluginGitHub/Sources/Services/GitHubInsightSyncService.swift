...
/// 启动重复的 GitHub 发现任务。
public actor GitHubInsightSyncService: SuperLog {
    /// 状态栏视图模型使用的共享同步协调器。
    public static let shared = GitHubInsightSyncService()
    ...
    /// - Returns: 项目同步后的状态。
    public func syncIfNeeded(projectPath: String, force: Bool = false) async -> GitHubInsightSyncState {
        ...
        if syncingProjects.contains(normalizedPath) {
            GitHubPlugin.logger.info("\(Self.t)同步已在进行中，跳过重复任务：\(normalizedPath)")
            return .syncing
        }
        ...
        do {
            GitHubPlugin.logger.info("\(Self.t)开始同步 GitHub 生态知识库：\(normalizedPath)，force=\(force)")
            guard let profile = profiler.profile(projectPath: normalizedPath) else {
                GitHubPlugin.logger.error("\(Self.t)项目路径不可读，无法同步 GitHub 生态知识库：\(normalizedPath)")
                return .failed("Project path is not readable.")
            }
            GitHubPlugin.logger.info("\(Self.t)项目画像完成：language=\(profile.primaryLanguage ?? "Unknown")，frameworks=\(profile.frameworks.count)，dependencies=\(profile.dependencies.count)，keywords=\(profile.keywords.count)")
            ...
            GitHubPlugin.logger.info("\(Self.t)GitHub 生态知识库同步完成：\(normalizedPath)，缓存条目：\(entries.count)")
            await MainActor.run {
            ...
            GitHubPlugin.logger.error("\(Self.t)GitHub 生态知识库同步失败：\(normalizedPath)，错误：\(error.localizedDescription)")
    ...
    /// 返回当前缓存状态，不触发网络发现。
    public func cachedState(projectPath: String) async -> GitHubInsightSyncState {
...
extension Notification.Name {
    ...
    public static let githubInsightDidSync = Notification.Name("GitHubInsightDidSync")
}
