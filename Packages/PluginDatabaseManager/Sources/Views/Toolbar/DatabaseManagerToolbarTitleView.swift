...
///
/// 注意：DatabaseManagerPlugin 的容器 id 为字面量 `"database-manager"`
/// （并非插件 `id`），因此 `containerID` 由调用方传入，保持与
/// `viewContainers` / `activateContainer` 调用点一致。
///
/// 不订阅 workspace 服务的 `objectWillChange`，
/// 改为「快照 + 事件刷新」：init 读一次当前 `activeViewContainerID`，
/// 监听 `.activeViewContainerIDDidChange` 事件更新，
/// 避免切换容器后在其它容器中残留标题。
struct DatabaseManagerToolbarTitleView: View {
