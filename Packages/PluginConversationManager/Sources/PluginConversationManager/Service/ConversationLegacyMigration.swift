...
/// 通过 `V4ConversationReader` 读取 v4 旧库的历史会话,批量导入到 v5 的 `ConversationStore`。
/// 同时通过 `ConversationMigrationProgressStore` 回报进度,供设置视图显示。
///
/// 设计要点(遵守 v4→v5 迁移契约):
/// - **后台执行**:由 `Task { }` 驱动,不阻塞 onBoot 串行链,App 立即可用。
/// - **迁移策略开关** `policy`:`.once`(上线用,只迁一次)/ `.always`(测试用,每次启动
///   都迁,便于验证幂等)。幂等性始终由 `importSummaries` 的按 id 去重兜底。当前默认 `.once`。
/// - **吞错**:`do/catch` 捕获所有错误并记日志,**绝不向上抛** —— 因为 onBoot 是串行
///   调度,抛错会阻塞后续所有插件。
/// - **无 legacy 数据时跳过**:全新安装或迁移窗口期之后,reader 返回 nil / 无旧库。
...
public struct ConversationLegacyMigration: SuperLog {
    ...
    /// 迁移策略开关。
    ///
    /// - 测试期:可设为 `.always`,每次启动都跑迁移,便于反复验证幂等和正确性。
    /// - 生产:`.once`,只迁一次。
    public static var policy: MigrationPolicy = .once

    private let reader: V4ConversationReader?
    ...
    func run() async {
        ...
        guard let reader else {
            // 无 legacy reader(全新安装或迁移窗口期已过)
            if Self.verbose {
                Self.logger.info("\(Self.t)会话迁移跳过(无 legacy 数据,全新安装或迁移窗口期已过)")
            }
            return
        }

        guard reader.hasLegacyData() else {
            // 没有可迁移的旧数据
            if Self.verbose {
