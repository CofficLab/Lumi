...
public struct SystemMessage: View {
...
public struct LoadingLocalModelSystemMessageView: View {
    @LumiUI.LumiTheme private var theme: any LumiUITheme
    ...
    private var provider: LLMProviderInfo? {
        ...
        return MessageRendererRuntime.providerInfoProvider(id)
    }
    ...
    private func ramText(minRAM: Int) -> String {
        MessageRendererRuntime.languagePreference == .chinese ? "\(minRAM) GB RAM 最低" : "\(minRAM) GB RAM min"
    }
    ...
    private var statusText: String {
        if isLoading {
            return MessageRendererRuntime.languagePreference == .chinese ? "正在加载模型…" : "Loading model…"
        }
        return MessageRendererRuntime.languagePreference == .chinese ? "模型已就绪" : "Model ready"
    }
    ...
    private func loadLocalModelInfo() async {
        ...
        localModelInfo = await MessageRendererRuntime.localModelInfoProvider(providerId, modelId)
    }
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 12) {
                ...
                if info.supportsVision {
                    AppTag(
                        MessageRendererRuntime.languagePreference == .chinese ? "视觉" : "Vision",
                        systemImage: "eye",
                        style: .accent
                    )
                }
                if info.supportsTools {
                    AppTag(
                        MessageRendererRuntime.languagePreference == .chinese ? "工具" : "Tools",
