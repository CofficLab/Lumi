...
public enum CaptureName: Int8, CaseIterable, Sendable {
    ...
    public static func fromString(_ string: String?) -> CaptureName? { // swiftlint:disable:this cyclomatic_complexity
        ...
        if let capture = fromExactString(string) {
            return capture
        }
        guard let separatorIndex = string.firstIndex(of: ".") else {
            return nil
        }
        return fromExactString(String(string[..<separatorIndex]))
    }

    private static func fromExactString(_ string: String) -> CaptureName? { // swiftlint:disable:this cyclomatic_complexity
        switch string {
        case "include":
            return .include
        case "constructor":
