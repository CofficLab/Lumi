...
public enum SemanticIndexMetrics {
    ...
    public static func recordCacheHit(workspacePath: String, entryCount: Int?) {
        ...
        lock.unlock()
        logger.info("semantic_index_cache_hit workspace=\(workspacePath, privacy: .public) entries=\(entryCount ?? 0)")
    }
    ...
    public static func recordCacheMiss(workspacePath: String, reason: String) {
        ...
        cacheMisses += 1
        lock.unlock()
        logger.info("semantic_index_cache_miss workspace=\(workspacePath, privacy: .public) reason=\(reason, privacy: .public)")
    }
    ...
    public static func recordIndexCompleted(workspacePath: String, duration: TimeInterval, entryCount: Int) {
        ...
        lastEntryCount = entryCount
        lock.unlock()
        logger.info("semantic_index_complete workspace=\(workspacePath, privacy: .public) duration=\(duration, privacy: .public)s entries=\(entryCount)")
    }
