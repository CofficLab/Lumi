...
@testable import PluginOpenInGitHubDesktop
...
func pluginRegistersAndRemovesToolAndDocumentation() throws {
    ...
    let plugin = OpenInGitHubDesktopPlugin()

    #expect(plugin.id == "com.coffic.lumi.plugin.open-in-github-desktop")
    ...
    #expect(tools.allTools().map(\.name) == [OpenInTool.gitHubDesktop.toolName])
