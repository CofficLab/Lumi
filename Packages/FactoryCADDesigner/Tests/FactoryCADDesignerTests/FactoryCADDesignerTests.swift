...
@Suite("FactoryCADDesigner")
@MainActor
struct FactoryCADDesignerTests {
    @Test("专用宿主启动 CAD 工作区与完整工具集")
    func startsCADDesigner() throws {
        let kernel = try FactoryCADDesigner.makeKernel()

        #expect(kernel.isPluginRegistered(id: FactoryCADDesigner.cadDesignerPluginID))
        #expect(kernel.isPluginEnabled(id: FactoryCADDesigner.cadDesignerPluginID))
    ...
    func assemblesSharedViews() throws {
        let kernel = try FactoryCADDesigner.makeKernel()
        _ = try FactoryCADDesigner.makeMainView(kernel: kernel)
        _ = try FactoryCADDesigner.makeSettingsView(kernel: kernel)
    }
}
