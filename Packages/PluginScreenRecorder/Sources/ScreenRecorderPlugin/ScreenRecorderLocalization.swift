...
public enum ScreenRecorderLocalization {
    public enum Language { case english, chinese; static var current: Language { Locale.preferredLanguages.first?.hasPrefix("zh") == true ? .chinese : .english } }
    /// 按当前内核语言偏好返回文案。
    public static func localized(_ language: Language, en: String, zh: String) -> String {
        switch language {
        case .english: en
        case .chinese: zh
        }
    ...
    public static func string(_ en: String, _ zh: String) -> String {
        localized(Language.current, en: en, zh: zh)
    }
