...
- [ ] 实现路径 normalizer（将 Git 返回路径规范化为相对 `repoRootPath` 的 POSIX 路径，rename 记录新路径）。
- [ ] 复用或封装 LibGit2Swift status 获取；补齐 untracked / conflicted（若当前 `getDiffFileList` 不返回，需在 LibGit2Swift 能力层补方法）。
...
- [ ] 在 `EditorFileTreeRefreshCoordinator` 持有 `@Published gitStatusSnapshot`。
- [ ] 项目切换时重置旧 snapshot、检测 Git repo、启动一次状态刷新。
