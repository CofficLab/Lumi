...
The bounded pagination API is now covered by the follow-up commit `b9942517c`. The next optimization boundary is refresh-source deduplication: keep message-list history updates on message events, while sender/activity/streaming changes update only their own UI state.
