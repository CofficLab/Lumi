...
> 日期：2026-05-25
> 状态：迁移设计中
> 涉及范围：Core + LumiPluginKit + Plugins
...
App 中可以保留插件注册文件，用来决定哪些插件随 App 一起加载。但插件的核心功能不应该依赖 App Core，而应该依赖稳定的插件 SDK：`LumiPluginKit`。
...
`LumiPluginKit` 是插件开发 SDK，负责定义插件和 App Core 之间的稳定边界。

适合放入 `LumiPluginKit` 的内容：
...
原则是：`LumiPluginKit` 应该是插件 SDK，而不是 Core 的搬家版。
...
Plugin target
  ...
  - 负责把 PluginCore 适配到 LumiPluginKit 扩展点
```
...
LumiPluginKit  → LumiApp/Core
LumiPluginKit  → 具体插件 Package
...
把 Core 中已经被插件使用的协议和值类型迁到 `LumiPluginKit`，并确保访问级别为 `public`。
...
### 2. 让 Core 依赖 LumiPluginKit

Core 中所有插件协议和值类型引用都应切换为 `import LumiPluginKit`。

验收标准：

- Core 不再定义自己的 `SuperPlugin`
- Core 不再定义和 `LumiPluginKit` 重复的共享模型
...
### LumiPluginKit 变成新 Core

这是最大风险。

如果迁移时把 App 状态、窗口管理、具体业务服务都放进 `LumiPluginKit`，它会变成新的 Core，插件仍然无法独立。
...
当前 `LumiPluginKit` 的部分 API 暴露了 `AgentToolKit` 类型，例如 `SuperAgentTool`。这会导致插件为了实现工具仍需直接 `import AgentToolKit`。

可接受的过渡方案：

- 插件 Package 依赖 `LumiPluginKit` 和少量领域 Kit
- 暂时允许依赖 `AgentToolKit`

长期优化方向：

- 将插件开发常用的工具协议 facade 收敛到 `LumiPluginKit`
...
### Core 和 LumiPluginKit 双份类型并存

迁移过程中，Core 和 `LumiPluginKit` 中可能短期存在同名类型。
...
最终应统一到 `LumiPluginKit`。

## 验收标准

阶段性验收：

- `LumiPluginKit` 可以独立 `swift test`
- 至少一个插件 Package 可以独立 `swift test`
- 插件 Package 不依赖 `LumiApp/Core`
- App 通过注册文件加载插件 Package
- Core 中不再保留和 `LumiPluginKit` 重复的插件协议定义
...
- 插件之间通过 `LumiPluginKit` 或领域 Kit 协作，不通过 Core 间接耦合

## 推荐下一步

1. 以 `LumiPluginKit` 为准，清理 Core 中重复的插件协议和值类型
...
这个试点也暴露了一个架构问题：即使 `PluginWebFetch` 只是 Agent Tool 插件，依赖 `LumiPluginKit` 时仍会一起构建 `LumiUI`、`LLMKit` 等较重依赖。后续可以考虑把 `LumiPluginKit` 拆成更细 target，例如：
